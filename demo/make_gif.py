...
def main():
    ...
    parser.add_argument(
        "--outputs_dir", type=str, default=str(DEMO_DIR / "outputs")
    )
    parser.add_argument(
        "--out", type=str, default=str(DEMO_DIR / "assets" / "demo.gif")
    )
    parser.add_argument(
        "--width",
        type=int,
    ...
    montages = sorted(
        glob.glob(os.path.join(args.outputs_dir, "*", "combined.png"))
    )
    if not montages:
        sys.exit(
            f"No combined.png found under {args.outputs_dir} — "
            "run demo/run_demo.py first"
        )
