...
Reads per-scene moge_signals.h5 (keys: planarity, normal, depth_metric)
produced by save_moge_signals_planarity.py and writes per-scene planes.h5
with the schema evaluate_all_baselines.py consumes (key "planes" uint16,
0 = non-planar; "frame_ids").
...
These are the canonical parameters used across the benchmark
(see evaluate_all_baselines.py).

Scene sharding via --part_id / --num_parts (contiguous slices of the sorted
scene list), so this can be fanned out across SLURM jobs. All parts write to
...
# ── OUR-method segmentation params (match the benchmark's internal
# segmentation) ──
...
def segment_scene(sig_h5_path, out_h5_path, device):
    """Read one scene's moge_signals.h5, segment every frame, write
    planes.h5."""
    with h5py.File(sig_h5_path, "r") as f:
        frame_ids = [
            x.decode() if isinstance(x, bytes) else x for x in f["frame_ids"][:]
        ]
        N = len(frame_ids)
        ...
        for i in range(N):
            ...
            if labels.max() >= 65536:
                raise ValueError(
                    f"{sig_h5_path} frame {i}: {labels.max()} labels "
...
def main():
    ...
    ap.add_argument(
        "--output_root", required=True, help="Dir to write <scene>/planes.h5"
    )
    ...
    if args.num_parts < 1 or not (0 <= args.part_id < args.num_parts):
        ap.error(
            f"--part_id {args.part_id} / --num_parts {args.num_parts}: "
    ...
    scenes = sorted(
        [
            ...
            if os.path.isfile(
                os.path.join(args.input_root, d, "moge_signals.h5")
            )
        ]
    )
    if not scenes:
    ...
    print(
        f"[INFO] part {args.part_id}/{args.num_parts}: "
        f"{len(part)}/{len(scenes)} scenes "
        f"(seg=compute_planar_segments plan>{THRESHOLD_PLANARITY} "
        f"n<{NORMAL_THRESHOLD_DEG}° "
        f"d_rel<{DEPTH_THRESHOLD_REL} match≥{NEIGHBOR_MATCH_COUNT}, "
    ...
    print(
        f"[DONE] part {args.part_id}: {total} frames, "
        f"{len(part)} scenes in {dt:.1f}s "
        f"({total / dt:.1f} fps)"
        if dt > 0
        else f"[DONE] part {args.part_id}"
    )
