...
Usage:
    python pxwplanar/inference/planarity/export_hf_checkpoint.py \
        [--src <model_epoch1.pt>] --out <dir>
"""
...
def main():
    ...
    print(
        f"       {len(state_dict)} tensors; epoch={ckpt.get('epoch')} "
        f"val_loss={ckpt.get('val_loss'):.4f} "
        f"val_accuracy={ckpt.get('val_accuracy'):.4f}"
    )
    ...
    if src_keys != new_keys:
        sys.exit(
            f"[FAIL] key sets differ: "
            f"missing={sorted(src_keys - new_keys)[:5]} "
    ...
    print(
        f"[OK] {len(src_keys)} tensors bitwise-identical; "
        "planarity_head present; "
        f"{size_gb:.2f} GB"
    )
