...
Usage:
    python run_inference.py --model_path /path/to/model.pt \
        --input_dir /path/to/images --output_dir /path/to/output
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python run_inference.py --model_path model.pt \\
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default 1600, matching the benchmark)"
        ),
    )
    parser.add_argument(
        "--extensions",
        ...
        help=(
            "Comma-separated image extensions to process "
            "(default: jpg,jpeg,png)"
        ),
    )
    ...
    # Initialize model (base MoGe weights come from the standard
    # HuggingFace cache)
    ...
    for ext in extensions:
        ...
        image_paths.extend(
            glob.glob(os.path.join(args.input_dir, f"*.{ext.upper()}"))
        )
    ...
    for image_path in tqdm(image_paths, desc="Processing"):
        ...
        try:
            ...
            if "planarity_probability_full" in results:
                ...
                results["planarity_binary_full"]
            else:
                prob = results["planarity_probability"]
                results["planarity_binary"]

            # Save raw probability
            if args.save_raw:
                np.save(
                    os.path.join(
                        args.output_dir, "raw", f"{base_name}_planarity.npy"
                    ),
                    prob,
                )
            ...
            if args.save_binary:
                ...
                cv2.imwrite(
                    os.path.join(
                        args.output_dir, "binary", f"{base_name}_binary.png"
                    ),
                    binary_mask,
                )
            ...
            if args.save_visualization:
                ...
                fig = model.visualize_prediction(
                    ...
                    save_path=os.path.join(
                        args.output_dir, "vis", f"{base_name}_vis.png"
                    ),
                    show_overlay=True,
                )
