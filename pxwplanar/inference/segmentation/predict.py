...
Runs full pipeline: RGB → MoGe (planarity + depth + normals) →
plane segmentation.

Usage:
    python predict.py --model_path /path/to/model.pt \
        --input_root /path/to/scannet --output_root /path/to/output
"""
...
def process_scene(scene_id, image_list, inference_model, output_dir, args):
    ...
    for idx in range(0, len(image_list), args.frame_skip):
        ...
        # Run MoGe inference (metric variant: focal/shift recovery +
        # metric scale, same as the benchmark pipeline)
        res = inference_model.predict_metric(
        ...
        normal = cv2.resize(
            normal.astype(np.float32), (W, H), interpolation=cv2.INTER_LINEAR
        )
        ...
        if args.save_visualization:
            segvis_save_path = os.path.join(
                segvis_dir, f"{base_name}_seg_vis.png"
            )
            ...
            n = len(np.unique(filtered_segmentation)) - (
                1 if (filtered_segmentation == 0).any() else 0
            )
            n1 = min(10, max(n, 1))
            seg_vis = visualize_top_components(
                filtered_segmentation, k=n1, return_colors=True
            )
            ...
            n2 = max(n // 2, 1)
            seg_vis2 = visualize_top_components(
                filtered_segmentation, k=n2, return_colors=True
            )
...
def main():
    ...
    for scene_id in tqdm(scene_id_list, desc="Processing scenes"):
        ...
        process_scene(
            scene_id, image_list, inference_model, args.output_root, args
        )
