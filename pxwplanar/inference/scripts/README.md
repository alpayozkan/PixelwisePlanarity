...
`alpayozkan/pxwplanar-moge2-planarity` when the local file is absent) — since
it is the first positional argument, pass an empty string (`""`) to use the
default while still supplying the later arguments. HF downloads land in the
standard cache (`~/.cache/huggingface`; override via `HF_HOME`).
