...
RG + EM + Saturation + Policies + Recovery + Last-stage RG +
Recursive Large Split
...
try:
    ...
    def TQDM(iterable=None, total=None, desc=None, disable=False, **kw):
        return _tqdm(
            iterable=iterable, total=total, desc=desc, disable=disable, **kw
        )
...
def _exclude_small_faces(
    faces: np.ndarray, FA: np.ndarray, thr: float
) -> np.ndarray:
...
def _read_legend_csv(legend_csv_path: str):
    ...
    with open(legend_csv_path, newline="") as f:
        reader = csv.DictReader(f)
        need = {"semantic_id", "semantic_name", "R", "G", "B"}
        missing = need - set(reader.fieldnames or [])
        if missing:
            sys.exit(
                f"[ERR] legend CSV missing columns: {sorted(list(missing))}"
            )
        for row in reader:
            try:
...
def _labels_from_face_colors(
    mesh: trimesh.Trimesh, legend_csv_path: str
) -> tuple[np.ndarray, dict[int, str]]:
    color2info, int2raw = _read_legend_csv(legend_csv_path)
    if (
        not hasattr(mesh, "visual")
        or mesh.visual is None
        or mesh.visual.face_colors is None
    ):
        sys.exit(
            "[ERR] Mesh has no face colors. "
            "Load mesh_by_semantic.ply (per-face colors).]"
        )
    ...
    if FC.shape[0] not in (mesh.faces.shape[0], mesh.vertices.shape[0]):
        sys.exit(
            "[ERR] face_colors length mismatch; "
            "are colors per-face in the PLY?]"
        )
    if FC.shape[0] == mesh.vertices.shape[0]:
        PWRITE(
            "[WARN] Colors appear per-vertex; "
            "projecting to per-face via triangle vertex[0]."
        )
        v0 = mesh.faces[:, 0]
        FC = FC[v0]
    ...
    lab_for_uk_sorted = np.array(
        [lut[int(k)] for k in uk_sorted], dtype=np.int32
    )
...
def fit_plane_from_faces_3pt(
    face_ids: np.ndarray,
    F: np.ndarray,
    V: np.ndarray,
    FA: np.ndarray | None = None,
):
    """Pick one triangle among face_ids (largest area if FA provided) and
    return its 3‑point plane."""
...
if NUMBA:
    ...
    @njit(cache=True)
    def _filter_local_consensus_csr_numba(
        face_ids, indptr, indices, min_nbrs, nfaces
    ):
...
def filter_local_consensus(
    ...
    if face_ids.size == 0:
        return face_ids
    if NUMBA:
        return _filter_local_consensus_csr_numba(
            face_ids, indptr, indices, int(min_nbrs), int(nfaces)
        )
...
def split_by_plane_offset_clusters(
    face_ids: np.ndarray, Cface: np.ndarray, n: np.ndarray, d: float, gap=0.012
):
    face_ids = _ensure_face_index_vector(face_ids)
    if face_ids.size == 0:
        return []
...
def eval_plane_quality(
    ...
    ok = (
        (p95_res <= p95_max)
        and (inlier_frac >= inlier_frac_min)
        and (thickness <= thickness_max)
        and (normal_p95 <= normal_p95_deg_max)
        and (min_width >= min_width_m)
        and (fill >= fill_frac_min)
    )
    return ok, metrics
...
def ransac_plane_over_faces(
    ...
    best_inliers = np.empty(0, np.int32)
    best_pl = None
    C = Cface[face_ids]
    Nf = FN[face_ids]
    for _ in range(max(1, int(max_iters))):
        idx = rng.choice(verts, size=3, replace=False)
        pl = _plane_from_three_points(
            V[int(idx[0])], V[int(idx[1])], V[int(idx[2])]
        )
        if pl is None:
            continue
        n, d = pl[:3], float(pl[3])
...
def _components_in_subset(
    sub_idx: np.ndarray, adj_lists: list[np.ndarray], nfaces: int
) -> list[np.ndarray]:
...
def grow_regions_in_label(
    ...
    faces_in_label = _ensure_face_index_vector(faces_in_label)
    in_label = np.zeros(F.shape[0], bool)
    in_label[faces_in_label] = True
    ...
    for seed in order:
        ...
        while stack:
            u = int(stack.pop())
            if visited_mask_label[u]:
                continue
            reg.append(u)
            cnt_since_refit += 1
            if cnt_since_refit >= params["rg_refit_every"]:
                # re-anchor plane to the largest-area face currently in the
                # region (3‑point)
                pl2 = fit_plane_from_faces_3pt(
                    np.array(reg, np.int32), F, V, FA
                )
                if pl2 is not None:
            ...
            for v in face_adj_lists[u]:
                v = int(v)
                if not in_label[v] or visited_mask_label[v]:
                    continue
                Nv = FN_gate[v]
                if abs(Nu @ Nv) < cos_dihedral:
                    continue
                if abs(Nv @ n) < cos_theta:
                    continue
                if gate_mode == "centroid":
                    if abs(Cface[v] @ n + d) > params["rg_dist_m"]:
                        continue
                elif gate_mode == "kof3":
                    ...
                    if (dist3 <= params["rg_dist_m"]).sum() < 2:
                        continue
                in_label[v] = False
                stack.append(v)
...
def merge_within_label(
    ...
    while changed:
        changed = False
        used = np.zeros(len(patches_label), bool)
        out = []
        for i in range(len(patches_label)):
            ...
            for j in range(i + 1, len(patches_label)):
                ...
                if abs(n1 @ n2) < cos_merge:
                    continue
                vA = np.unique(F[pi["faces"]].reshape(-1))
                vB = np.unique(F[pj["faces"]].reshape(-1))
                dA2B = (
                    np.percentile(np.abs(V[vA] @ n2 + d2), 85)
                    if vA.size > 0
                    else np.inf
                )
                dB2A = (
                    np.percentile(np.abs(V[vB] @ n1 + d1), 85)
                    if vB.size > 0
                    else np.inf
                )
                if max(dA2B, dB2A) > merge_dist_m:
                    continue
                faces_union = _ensure_face_index_vector(
                    np.concatenate([pi["faces"], pj["faces"]])
                )
                ok, plane, _ = fit_and_quality(
                    faces_union,
                    F,
                    V,
                    FN,
                    FA,
                    qual_params["p95_final_max"],
                    qual_params["inlier_frac_min"],
                    sweep_dist_m,
                    qual_params["normal_p95_deg_max"],
                    qual_params["thickness_max"],
                    qual_params["min_width_m"],
                ...
                sweep = filter_local_consensus(
                    sweep, indptr, indices, min_nbrs=2, nfaces=nfaces
                )
                faces_union = _ensure_face_index_vector(
                    np.concatenate([faces_union, sweep])
                )
                ok2, plane2, _ = fit_and_quality(
                    faces_union,
                    F,
                    V,
                    FN,
                ...
                if ok2:
                    pi = dict(
                        ...
                        label_int=pi["label_int"],
                        label_raw=pi["label_raw"],
                        alg="Merge",
                    )
                    used[j] = True
                    changed = True
...
def saturate_within_label(
    ...
    for _ in range(max(1, rounds)):
        ...
        for p in patches_label:
            ...
            sweep = filter_local_consensus(
                sweep, indptr, indices, min_nbrs=1, nfaces=nfaces
            )
            union = _ensure_face_index_vector(
                np.concatenate([p["faces"], sweep])
            )
            ok, plane2, _ = fit_and_quality(
                union,
                F,
                V,
                FN,
        ...
        for pid, p in enumerate(patches_label):
            f = p["faces"]
            keep = f[face2pid[f] == pid]
            if keep.size == 0:
                continue
            p["faces"] = keep
            p["area"] = float(FA[keep].sum())
            new_list.append(p)
        ...
        patches_label = merge_within_label(
            ...
            indices,
            nfaces,
            merge_theta_deg=10.0,
            merge_dist_m=max(0.02, 0.6 * sat_dist_m),
            sweep_normal_deg=sat_normal_deg,
            sweep_dist_m=sat_dist_m,
            sweep_frac_vertices=sat_frac_vertices,
            faces_in_label=faces_in_label,
            FN=FN,
            face_adj_lists=face_adj_lists,
...
def compute_face_tiers4(
    ...
    return (
        small.astype(np.int32),
        normal.astype(np.int32),
        big.astype(np.int32),
        huge.astype(np.int32),
    )


def seed_huge_faces_as_planes(
    lbl: int,
...
def extract_big_patches_in_label(
    lbl: int,
    ...
    for comp in comps:
        ...
        while leftover.size >= max(1, args.big_min_faces_patch):
            if int(args.big_ransac_enable) != 1:
                break
            if planes_made >= int(
                args.big_ransac_max_planes_per_comp
                if hasattr(args, "big_ransac_max_planes_per_comp")
                else 16
            ):
                break
            ...
            if pl is None or inliers.size < max(
                args.big_min_faces_patch,
                int(np.ceil(leftover.size * args.big_ransac_min_inlier_frac)),
            ):
                break
            n, d = pl[:3], float(pl[3])
            ok, _ = eval_plane_quality(
                inliers,
                n,
                d,
                F,
            ...
            if ok and area >= float(args.big_min_area_patch):
                ...
                mask = np.array(
                    [int(f) not in in_set for f in leftover], dtype=bool
                )
...
def process_one_label_normal_only(
    lbl: int,
    F: np.ndarray,
    V: np.ndarray,
    FA: np.ndarray,
    ...
    nfaces: int,
    args,
    faces_override: np.ndarray | None = None,
) -> list[dict]:
    faces_in_label = (
        faces_override
        if faces_override is not None
        else np.where(labels_f == int(lbl))[0]
    )
    faces_in_label = _ensure_face_index_vector(faces_in_label)
    if faces_in_label.size < max(20, args.min_faces_patch // 2):
        return []
    label_raw = str(int2raw.get(int(lbl), "unannotated"))
    ...
    regs = grow_regions_in_label(
        ...
        Cface=Cface,
        params=dict(
            rg_dihedral_deg=args.rg_dihedral_deg,
    ...
    taken_label[labels_f != int(lbl)] = True
    for reg in regs_sorted:
        reg = _ensure_face_index_vector(reg)
        if taken_label[reg].all():
            continue
        ...
        for _ in range(max(1, args.em_max_iters)):
            ...
            sweep = sweep_inliers_label(
                ...
                FN,
                Cface,
                args.sweep_normal_deg,
                args.sweep_dist_m,
                args.sweep_frac_vertices,
                cand_mask,
            ...
            clusters = split_by_plane_offset_clusters(
                union, Cface, n, d, gap=max(0.010, 0.5 * args.sweep_dist_m)
            )
            if clusters:
                ...
                for cl in clusters:
                    ov = len(
                        seed_set.intersection(set(int(x) for x in cl.tolist()))
                    )
                    if ov > best_ov:
                        best, best_ov = cl, ov
                union = best
            ...
            growth = (
                0.0
                if prev_sz == 0
                else (union.size - prev_sz) / max(1, prev_sz)
            )
            curr = union
            if growth < args.em_min_growth:
                break
            prev_sz = union.size

        if (
            curr.size >= args.min_faces_patch
            and float(FA[curr].sum()) >= args.min_area_patch
        ):
    ...
    qualA = dict(
        p95_final_max=args.p95_final_max,
        ...
        fill_frac_min=args.fill_frac_min,
    )

    label_patches = merge_within_label(
        label_patches,
        F,
        V,
        FA,
    ...
    qualSat = dict(
        ...
        min_width_m=args.sat_min_width_m,
        fill_frac_min=args.sat_fill_frac_min,
    )

    label_patches = saturate_within_label(
        label_patches,
        faces_in_label,
        F,
        V,
        ...
        indices,
        nfaces,
        sat_normal_deg=args.sat_normal_deg,
        sat_dist_m=args.sat_dist_m,
        sat_frac_vertices=args.sat_frac_vertices,
        qual_params=qualSat,
        rounds=args.sat_rounds,
        gate_mode=args.gate_mode,
        irls_max_iters=args.irls_max_iters,
        irls_eps=args.irls_eps,
    )
...
def process_label_with_tiers(
    ...
    faces_for_growth = _exclude_small_faces(
        all_faces, FA, args.tier_small_area_m2
    )

    huge_patches = seed_huge_faces_as_planes(
        lbl, huge_faces, F, V, FN, FA, args, int2raw
    )
    big_patches = extract_big_patches_in_label(
        lbl, big_faces, F, V, FN, FA, face_adj_lists, Cface, args, int2raw
    )
    ...
    if patches:
        ...
        patches = merge_within_label(
            ...
            sweep_dist_m=args.sweep_dist_m,
            sweep_frac_vertices=args.sweep_frac_vertices,
            faces_in_label=faces_for_growth,
            FN=FN,
            face_adj_lists=face_adj_lists,
            qual_params=qualA,
            gate_mode=args.gate_mode,
        ...
        patches = saturate_within_label(
            ...
            sat_frac_vertices=args.sat_frac_vertices,
            qual_params=qualSat,
            rounds=max(1, args.sat_rounds // 2),
            gate_mode=args.gate_mode,
            irls_max_iters=args.irls_max_iters,
            irls_eps=args.irls_eps,
        )
...
def _refit_plane_from_faces(
    face_ids: np.ndarray, F: np.ndarray, V: np.ndarray, FA: np.ndarray
):
    face_ids = _ensure_face_index_vector(face_ids)
    if face_ids.size == 0:
        return None
...
def rebuild_planes_from_face_pid(
    face_pid: np.ndarray,
    F: np.ndarray,
    V: np.ndarray,
    FA: np.ndarray,
    ...
    prev_planes_meta: dict[int, dict] | None = None,
):
    present = np.unique(face_pid[face_pid >= 0])
    if present.size == 0:
        return face_pid, []
    areas_by_old = {
        int(pid): float(FA[face_pid == int(pid)].sum()) for pid in present
    }
...
def last_stage_relaxed_rg(
    ...
    pid2label = {
        int(p["plane_id"]): int(p.get("label_int", -1)) for p in planes_meta
    }
    base_area = {
        int(pid): float(FA[face_pid == int(pid)].sum()) for pid in unique_pids
    }
    order = sorted(unique_pids.tolist(), key=lambda x: -base_area[int(x)])

    cos_last = float(np.cos(np.deg2rad(normal_deg)))
    pbar = TQDM(
        order, desc="Last-stage RG", disable=not progress, dynamic_ncols=True
    )
    thr = float(min_area_face_m2)
    ...
    for pid in pbar:
        ...
        pbar.set_postfix(
            pid=pid, cls=raw[:14], F=int(A_faces.size), V=int(A_verts)
        )
        ...
        for _ in range(max(0, rg_iters)):
            ...
            if not neigh:
                break
            neighbors = _ensure_face_index_vector(
                np.unique(np.concatenate(neigh))
            )
            maskA = np.zeros(nfaces, np.bool_)
            maskA[A_faces] = True
    ...
    return rebuild_planes_from_face_pid(
        face_pid,
        F,
        V,
        FA,
...
def final_small_claim_global(
    ...
    if int(args.small_claim_require_adjacent) == 1:
        pid2faceset = {
            int(pid): set(np.where(face_pid == int(pid))[0].tolist())
            for pid in np.unique(face_pid[face_pid >= 0])
        }

    candidates = np.where((face_pid < 0) & (labels_f >= 0) & (small_thr > FA))[
        0
    ].astype(np.int32)
    added = 0
    for f in candidates:
        ...
        for pid in pids:
            ...
            if abs(nf @ n) < cos_n:
                continue
            dist = abs(cf @ n + d)
            if dist > float(args.small_claim_dist_m):
                continue
            if pid2faceset is not None:
                neigh = face_adj_lists[int(f)]
                s = pid2faceset[int(pid)]
            ...
            if dist < best_dist:
                ...
                best_pid = int(pid)
        if best_pid >= 0:
            face_pid[int(f)] = best_pid
            added += 1

    if added > 0:
        PWRITE(
            f"[SMALL-CLAIM] Added {added} small faces to existing planes "
            "(final stage)."
        )
...
def post_ransac_unlabeled_simple(
    face_pid: np.ndarray,
    planes_meta: list[dict],
    F: np.ndarray,
    V: np.ndarray,
    ...
    for lbl in pbar:
        ...
        for comp in comps:
            ...
            while rem.size >= max(3, int(args.post_min_faces_patch)):
                if planes_made >= int(args.post_ransac_max_planes_per_comp):
                    break
                pl, inliers = ransac_plane_over_faces(
                    rem,
                    F,
                ...
                if pl is None:
                    break
                if inliers.size < int(args.post_min_faces_patch):
                    break
                frac = float(inliers.size) / float(rem.size)
                if frac < float(args.post_ransac_min_inlier_frac):
                    break
                area_here = float(FA[inliers].sum())
                if area_here < float(args.post_min_area_m2):
                    break
...
# -------------------- save helpers --------------------
def _write_ply_with_face_pid_binary(
    vertices, faces, face_pid, labels_f, out_path, planes_json=None
):
    import struct
    ...
    with open(out_path, "wb") as f:
        ...
        if planes_json is not None:
            for p in planes_json:
                ...
                d = float(p.get("d", 0.0))
                lab = int(p.get("label_int", -1))
                labraw = str(p.get("label_raw", ""))
                col = p.get("color_rgb", [0, 0, 0])
                area = float(p.get("area", 0.0))
                header.append(
                    f"comment plane_id {pid_i} label_int {lab} "
                    f'label_raw "{labraw}" '
                    f"n {n[0]} {n[1]} {n[2]} d {d} area {area} "
...
def split_label_faces_into_two(
    faces_in_label: np.ndarray, Cface: np.ndarray, mode: str = "axis"
) -> tuple[np.ndarray, np.ndarray]:
    faces_in_label = _ensure_face_index_vector(faces_in_label)
        ...
        if have_id:
            ...
            if needed_max_id >= len(names):
                names.extend(
                    [
                        f"unknown_{i}"
                        for i in range(len(names), needed_max_id + 1)
                    ]
                )
            return names
...
def build_mesh_and_legend_from_h5(args):
    ...
    group_names = _read_group_names_csv(
        args.groups_csv, needed_max_id=int(GI.max())
    )
    raw_names = [group_names[int(i)] for i in GI]
    sem_names = (
        [_normalize_sem_name(n) for n in raw_names]
        if int(args.normalize_group_names) == 1
        else raw_names
    )
    ...
    sem_to_color = {
        n: rng.integers(0, 256, size=3, dtype=np.uint8) for n in uniq
    }
...
def merge_planes_by_params(
    ...
    for i in range(len(pids)):
        for j in range(i + 1, len(pids)):
            ...
            if debug:
                PWRITE(
                    f"[PARAM-MERGE] pid={pids[i]} ↔ pid={pids[j]} "
    ...
    print(
        f"[MERGE-PARAMS] Planes before={num_before} after={num_after} "
...
def write_normals_log(
    ...
    NOTE: Aggregated per-label fits also use the 3‑point rule:
          pick the largest-area face of the label and compute from its
          3 vertices.
    """
    os.makedirs(out_dir, exist_ok=True)
    ...
    with open(path, mode) as f:
        ...
        if include_per_plane and planes_meta:
            ...
            for lab in sorted(by_label.keys()):
                ...
                planes = sorted(
                    by_label[lab], key=lambda q: -float(q.get("area", 0.0))
                )
                ...
                for p in planes:
                    ...
                    f.write(
                        ...
                        f"d={d:+.6f}  faces={faces:7d}  "
                        f"area={area:.6f}  alg={alg}\n"
                    )
                f.write("\n")
        ...
        if (
            ...
            and (F is not None)
            and (V is not None)
            and (labels_f is not None)
        ):
            uniq = np.unique(labels_f[labels_f >= 0])
            f.write(
                "# Aggregated per-label fits "
                "(largest-area face per label; 3‑point)\n"
            )
...
# -------------------- Scaling helpers --------------------
BASE_MPAU = (
    0.02539999969303608  # meters per asset unit for the original tuned scene
)


def _read_meters_per_asset_unit_from_scene_csv(path: str) -> float | None:
    """
    Expect CSV:
...
def _apply_scaling_to_args(args, mode: str, mpau_scene: float):
    ...
    if mode == "off":
        ...
        return dict(
            ...
            area=[],
            info=dict(
                mode=mode, Sdist=1.0, Sarea=1.0, base=BASE_MPAU, mpau=mpau_scene
            ),
        )
    ...
    # "original": thresholds tuned in *asset units* for BASE_MPAU, so keep
    # the physical length.  "metric": thresholds are in meters and convert
    # to mesh units.
    Sdist = BASE_MPAU / mpau_scene if mode == "original" else 1.0 / mpau_scene
    ...
    print(
        f"[SCALE] BASE_MPAU={base:.12f} m/unit   "
        f"MPAU_scene={mpau_scene:.12f} m/unit"
    )
    ...
    return dict(
        ...
        area=changed_area,
        info=dict(
            mode=mode, Sdist=Sdist, Sarea=Sarea, base=BASE_MPAU, mpau=mpau_scene
        ),
    )
...
def main(args):
    ...
    #     "Planes with per-face labels, POST-RANSAC, final param-merge, "
    #     "normals.log (3‑point fits) + unit-aware scaling"
    # )

    # # I/O (PLY+legend)
    # ap.add_argument("--mesh", type=str, default="",
    #                 help="mesh_by_semantic.ply (per-face RGB encodes label).")
    # ap.add_argument("--legend_csv", type=str, default="",
    #                 help="semantic_legend.csv "
    #                      "(maps RGB -> semantic_id, name)")
    ...
    # ap.add_argument("--h5_verts", type=str, default="",
    #                 help="Path to mesh_vertices.hdf5")
    # ap.add_argument("--h5_faces", type=str, default="",
    #                 help="Path to mesh_faces_vi.hdf5")
    # ap.add_argument("--h5_face_groups", type=str, default="",
    #                 help="Path to mesh_faces_gi.hdf5")
    # ap.add_argument("--groups_csv", type=str, default="",
    #                 help="Path to metadata_groups.csv")
    ...
    # ap.add_argument("--large_split_mode", type=str,
    #                 choices=["axis","pca"], default="axis")
    ...
    #                 choices=["threads","processes"], default="threads")
    ...
    # ap.add_argument("--rg_gate_mode", type=str,
    #                 choices=["kof3","centroid","none"], default="kof3")
    ...
    # ap.add_argument("--gate_mode", type=str,
    #                 choices=["kof3","centroid","none"], default="kof3")
    ...
    # ap.add_argument("--merge_params_scope", type=str,
    #                 choices=["label","global"], default="label")
    ...
    # ap.add_argument("--area_hist_png_name", type=str,
    #                 default="face_area_hist.png")

    # # ---- NEW: Scaling controls ----
    # ap.add_argument("--scale", type=str,
    #                 choices=["original","metric","off"], default="original",
    #                 help="Unit scaling mode. 'original': parameters tuned "
    #                      "for BASE_MPAU are rescaled to scene units. "
    #                      "'metric': parameters are in meters and converted "
    #                      "to scene units. 'off': no scaling.")
    # ap.add_argument("--scene_csv", type=str, default="",
    #                 help="Path to metadata_scene.csv "
    #                      "(to read meters_per_asset_unit).")
    # ap.add_argument("--meters_per_asset_unit", type=float, default=-1.0,
    #                 help="Override meters_per_asset_unit; "
    #                      "if >0, overrides --scene_csv.")

    # args = ap.parse_args()
    os.makedirs(args.out, exist_ok=True)
    show_bar = bool(args.progress)

    # ---- Read scene scale (MPAU) & apply parameter scaling BEFORE any
    #      processing ----
    mpau_scene = (
        float(args.meters_per_asset_unit)
        if args.meters_per_asset_unit and args.meters_per_asset_unit > 0
        else None
    )
    if mpau_scene is None:
        mpau_scene = (
            _read_meters_per_asset_unit_from_scene_csv(args.scene_csv) or -1.0
        )
    ...
    use_h5 = bool(
        args.h5_verts
        and args.h5_faces
        and args.h5_face_groups
        and args.groups_csv
    )
    if not (use_ply or use_h5):
        sys.exit(
            "[ERR] Provide either --mesh + --legend_csv, or the HDF5 quartet "
    ...
    if not legend_csv_path:
        sys.exit(
            "[ERR] --legend_csv is required "
            "(or let the HDF5 path auto-generate it)."
        )
    ...
    need_triang = (
        (F_raw.dtype == object) or (F_raw.ndim != 2) or (F_raw.shape[1] != 3)
    )
    if need_triang:
        newF, newC = [], []
    ...
    print(
        f"[CLEAN] Triangulated mesh: V={mesh.vertices.shape[0]:,}  "
        f"F={mesh.faces.shape[0]:,}"
    )
    ...
    assert F.ndim == 2 and F.shape[1] == 3, (
        f"[ERR] Faces must be triangles (N,3). Got {F.shape}"
    )
    ...
    for a, b in adj_pairs.astype(np.int32):
        face_adj_lists[int(a)].append(int(b))
        face_adj_lists[int(b)].append(int(a))
    face_adj_lists = [
        np.array(x, np.int32) if x else np.empty(0, np.int32)
        for x in face_adj_lists
    ]
    ...
    single_plane_set = parse_label_list(
        args.policy_single_plane_labels, int2raw
    )
    ...
    if single_plane_set & skip_set:
        ...
        PWRITE(
            f"[WARN] Labels present in both single-plane and skip sets; "
            f"will be SKIPPED: {both}"
        )
    ...
    if single_plane_set:
        bar_policy = TQDM(
            sorted(single_plane_set),
            desc="Policy single-plane",
        ...
        for lbl in bar_policy:
            if int(lbl) in skip_set:
                ...
                continue
            faces_in_label = _ensure_face_index_vector(
                np.where(labels_f == int(lbl))[0]
            )
            raw = str(int2raw.get(int(lbl), ""))
            bar_policy.set_postfix(
                id=int(lbl),
                cls=raw[:14],
                F=int(faces_in_label.size),
                V=int(np.unique(F[faces_in_label].reshape(-1)).size),
            )
            if faces_in_label.size == 0:
                PWRITE(
                    f"[POLICY] Single-plane label {lbl} has 0 faces; skipping."
                )
                continue
            faces_for_policy = _exclude_small_faces(
                faces_in_label, FA, args.tier_small_area_m2
            )
            if faces_for_policy.size == 0:
                PWRITE(
                    f"[POLICY] Single-plane label {lbl}: "
                    "all faces are small → skip."
                )
                continue
            pl = fit_plane_from_faces_3pt(faces_for_policy, F, V, FA)
            if pl is None:
                PWRITE(
                    f"[POLICY] Could not fit plane for label {lbl}; skipping."
                )
                continue
            n, d = pl[:3], float(pl[3])
            area = float(FA[faces_for_policy].sum())
            patches_all.append(
                dict(
            ...
            PWRITE(
                f"[POLICY] {raw} (id={lbl})  "
                f"F={faces_in_label.size:,}  area={area:.3f}"
            )
    ...
    process_labels = [
        ...
        if int(x) not in skip_set and int(x) not in single_plane_set
    ]
    ...
    for lbl in process_labels:
        ...
        info = {
            "faces": int(faces_in_label.size),
            "verts": verts_count,
            "area": float(FA[faces_in_label].sum()),
            "raw": raw,
        }
        if args.large_split_enable and verts_count >= int(
            args.large_split_verts
        ):
            if args.large_split_recursive:
                ...
                PWRITE(
                    f"[SPLIT] {raw} (id={int(lbl)}) "
                    f"verts={verts_count:,} >= {args.large_split_verts} "
                    f"→ recursive parts={len(parts)} "
                    f"(min_faces={args.large_split_min_faces}, "
                    f"max_parts={args.large_split_max_parts})"
                )
                ...
                for part_idx, part_faces in enumerate(parts):
                    ...
                    PWRITE(
                        f"▶ Start  {raw} (id={int(lbl)})  "
                        f"F={info['faces']:,}  V={info['verts']:,}  "
                        f"[subset #{part_idx + 1} "
                        f"F={part_faces.size:,} V={sub_verts:,}]"
                    )
                    work_items.append(
                        dict(
                            lbl=int(lbl),
                ...
                A, B = split_label_faces_into_two(
                    faces_in_label, Cface, mode=args.large_split_mode
                )
                split_labels.add(int(lbl))
                PWRITE(
                    f"[SPLIT] {raw} (id={int(lbl)})  "
            ...
            work_items.append(
                dict(lbl=int(lbl), faces=None, info=info, subV=info["verts"])
            )

    Executor = (
        ThreadPoolExecutor if args.backend == "threads" else ProcessPoolExecutor
    )
    if args.jobs > 1 and len(work_items) > 1:
        with Executor(max_workers=args.jobs) as ex:
            ...
            for wi in work_items:
                ...
                PWRITE(
                    f"▶ Start  {info['raw']} (id={lbl})  "
                    f"F={info['faces']:,}  V={info['verts']:,}"
                    + (
                        ""
                        if faces_sub is None
                        else f"  [subset F={faces_sub.size:,} V={subV:,}]"
                    )
                )
                fut = ex.submit(
                    process_label_with_tiers,
                    lbl,
                    F,
                    V,
                    FA,
                ...
                futures.append(fut)
                meta[fut] = (lbl, info, faces_sub, subV)
            pbar = TQDM(
                total=len(futures),
                desc=f"Labels (pass A+tiers, {args.backend})",
                disable=not show_bar,
                dynamic_ncols=True,
            )
        ...
        for wi in pbarA:
            ...
            subV = wi.get("subV", info["verts"])
            pbarA.set_postfix(
                id=lbl, cls=info["raw"][:14], F=info["faces"], V=info["verts"]
            )
            try:
                patches_all.extend(
                    process_label_with_tiers(
            ...
            if faces_sub is not None:
                PWRITE(
                    f"[DONE]  {info['raw']} (id={lbl}) subset "
                    f"F={faces_sub.size:,} V={subV:,}"
                )
    ...
    if args.recover_enable:
        ...
        for p in patches_all:
            covered[p["faces"]] = True
        labels_for_recovery = process_labels
        pbar_rec = TQDM(
            labels_for_recovery,
            desc="Labels (recovery)",
            disable=not show_bar,
            dynamic_ncols=True,
        )
        for lbl in pbar_rec:
            info = label_stats.get(
                int(lbl), {"faces": 0, "verts": 0, "area": 0.0, "raw": ""}
            )
            pbar_rec.set_postfix(
                id=int(lbl),
                cls=info["raw"][:14],
                F=info["faces"],
                V=info["verts"],
            ...
            if leftover.size < max(12, args.min_faces_patch_small // 2):
                continue

            leftover = _exclude_small_faces(
                leftover, FA, args.tier_small_area_m2
            )
            faces_for_growth = _exclude_small_faces(
                faces_in_label, FA, args.tier_small_area_m2
            )
            ...
            regs = grow_regions_in_label(
                ...
                Cface=Cface,
                params=dict(
                    rg_dihedral_deg=args.rg_dihedral_deg_small,
            ...
            taken_label = np.zeros(F.shape[0], bool)
            taken_label[labels_f != int(lbl)] = True
            taken_label[covered] = True
            areas = [FA[r].sum() for r in regs]
            order = (
                np.argsort(-np.asarray(areas))
                if areas
            ...
            for reg in regs_sorted:
                ...
                ok, plane, _ = fit_and_quality(
                    ...
                    args.min_width_m_small,
                    args.fill_frac_min_small,
                    args.irls_max_iters,
                    args.irls_eps,
                )
                if not ok:
                ...
                for _ in range(max(1, args.em_max_iters_small)):
                    ...
                    sweep = sweep_inliers_label(
                        ...
                        FN,
                        Cface,
                        args.sweep_normal_deg_small,
                        args.sweep_dist_m_small,
                        args.sweep_frac_vertices_small,
                        cand_mask,
                        gate_mode=args.gate_mode,
                    )
                    sweep = filter_local_consensus(
                    ...
                    curr = union
                    if growth < args.em_min_growth_small:
                        break
                    prev_sz = union.size

                if (
                    curr.size >= args.min_faces_patch_small
                    and float(FA[curr].sum()) >= args.min_area_patch_small
                ):
                    taken_label[curr] = True
                    covered[curr] = True
                    patches_all.append(
    ...
    # --- Freeze planes: build global face_pid + planes_meta
    #     (pre-last-stage) ---
    ...
    if args.last_enable and (np.max(face_pid) >= 0):
        face_pid, planes_meta = last_stage_relaxed_rg(
            ...
            face_adj_lists,
            progress=show_bar,
            dist_m=args.last_dist_m,
            normal_deg=args.last_normal_deg,
    ...
    def collect_patches_from_face_pid(
        face_pid: np.ndarray,
        planes_meta: list[dict],
        labels_f: np.ndarray,
        FA: np.ndarray,
    ...
    def face_pid_from_patches(patches: list[dict], nfaces: int) -> np.ndarray:
        ...
        patches_sorted = sorted(
            patches, key=lambda p: -float(p.get("area", 0.0))
        )
        pid = 0
    ...
    def final_merge_for_split_labels(
        face_pid: np.ndarray,
        planes_meta: list[dict],
        split_labels: set,
        labels_f: np.ndarray,
        ...
        if not split_labels:
            return face_pid, planes_meta
        PWRITE(
            f"[FINAL] Cross-split merge on labels: {sorted(list(split_labels))}"
        )
        patches_all = collect_patches_from_face_pid(
            face_pid, planes_meta, labels_f, FA
        )
        by_label: dict[int, list[dict]] = {}
        for p in patches_all:
            lab = int(p.get("label_int", -1))
        ...
        for lab, group in by_label.items():
            if lab not in split_labels:
                ...
                continue
            faces_in_label = _ensure_face_index_vector(
                np.where(labels_f == int(lab))[0]
            )
            faces_in_label = _exclude_small_faces(
                faces_in_label, FA, args.tier_small_area_m2
            )
            qualA = dict(
                p95_final_max=args.p95_final_max,
    ...
    if len(planes_meta) == 0:
        print(
            "[NOTE] No planes passed quality thresholds; "
            "consider relaxing post_ransac_* parameters."
        )
...
# SCENE=/Users/ahmetcanyavuz/scannetpp_gt/ai_001_002
# python3 planes_from_mesh_hypersim.py \
#   --h5_verts $SCENE/_detail/mesh/mesh_vertices.hdf5 \
#   --h5_faces $SCENE/_detail/mesh/mesh_faces_vi.hdf5 \
#   --h5_face_groups $SCENE/_detail/mesh/mesh_faces_gi.hdf5 \
#   --groups_csv $SCENE/_detail/mesh/metadata_groups.csv \
#   --scene_csv $SCENE/_detail/metadata_scene.csv \
#   --legend_csv $SCENE/planes_out/semantic_legend.csv \
#   --out $SCENE/planes_out \
#   --scale original \
#   --rg_theta_deg 8.0 --rg_dist_m 0.015 --rg_dihedral_deg 55.0 \
#   --rg_refit_every 15 \
...
#   --normal_p95_deg_max 8.0 --thickness_max_mul 1.6 --min_width_m 0.06 \
#   --fill_frac_min 0.18 \
#   --merge_theta_deg 10.0 --merge_dist_m 0.02 \
#   --recover_enable 1 \
#   --rg_theta_deg_small 12.0 --rg_dist_m_small 0.02 \
#   --rg_dihedral_deg_small 60.0 --rg_refit_every_small 20 \
#   --sweep_normal_deg_small 12.0 --sweep_dist_m_small 0.015 \
#   --sweep_frac_vertices_small 0.66 \
...
#   --large_split_verts 700000 --large_split_max_parts 8 \
...
#   --big_ransac_dist_m 0.010 --big_ransac_normal_deg 10.0 \
#   --big_ransac_min_inlier_frac 0.50 \
...
#   --merge_params_enable 1 --merge_params_scope global \
#   --merge_params_normal_deg 1 --merge_params_d_m 0.01 \
#   --merge_params_refit 1 --merge_params_debug 0 \
