#!/usr/bin/env python3
"""Find SYNTHIA scenes that are missing or incomplete from the generated
planes output.
...
from pxwplanar.paths import synthia_path, synthia_raw_path
...
for src_root, planes_root, split in [
    ...
    src_scenes = sorted(
        [
            s
            for s in os.listdir(src_root)
            if s.startswith("test5_")
            and os.path.isdir(os.path.join(src_root, s))
        ]
    )
    ...
    for scene in src_scenes:
        ...
        if not os.path.exists(json_path):
            ...
            if os.path.exists(h5_path):
                print(
                    f"[INCOMPLETE] {split}/{scene} "
                    "(h5 exists but no planes.json)"
                )
            else:
    ...
    if os.path.isdir(planes_root):
            ...
            [
                s
                for s in os.listdir(planes_root)
                if os.path.exists(os.path.join(planes_root, s, "planes.json"))
            ]
        )
    print(
        f"{split}: {len(src_scenes)} total, {done} done, "
        f"{len([m for m in missing if m[0] == split])} missing"
    )
