#!/usr/bin/env python3
"""Check how many SYNTHIA planar pixels were filtered by the old 300m
depth cap."""
    ...
    for scene_name in tqdm(scenes, desc=split):
        ...
        for ts in sorted(os.listdir(scene_dir)):
            ...
            for fname in sorted(os.listdir(os.path.join(ts_dir, "Depth"))):
                ...
                seg = np.array(
                    Image.open(os.path.join(ts_dir, "SemSeg", fname))
                )
            ...
            if total_lost > 0:
                ...
                print(
                    f"[{split}/{scene_name}] {n_frames} frames: "
                    f"{total_lost} planar px lost ({pct:.2f}%), "
                    f"max depth: {max_lost_depth:.1f}m"
                )
                affected.append((split, os.path.join(data_root, scene_name)))
            else:
                print(
                    f"[{split}/{scene_name}] {n_frames} frames: no pixels lost"
                )
