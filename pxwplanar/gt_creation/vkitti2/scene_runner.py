...
# Add project root to path for imports (also runnable directly, not just via -m)
...
from pxwplanar.shared.outdoor.ransac_image import (
    compute_normals,
    depth_to_xyz,
    extract_planes_from_frame,
)
...
PLANAR = {
    ...
    8,
    11,
}  # terrain, building, road, traffic_sign, misc (walls/sidewalks)
...
def load_extrinsics(data_root, scene, variant, camera=0):
    ...
    Each line: frame cameraID r1,1 r1,2 r1,3 t1 r2,1 r2,2 r2,3 t2
               r3,1 r3,2 r3,3 t3 0 0 0 1
...
def rgb_to_class_ids(seg_rgb):
    ...
    for color, (_name, cid) in CLASS_MAP.items():
...
def discover_frames(
    rgb_root, depth_root, semseg_root, scene, variant, camera=0
):
    """
    ...
    depth_dir = os.path.join(
        depth_root, scene, variant, "frames", "depth", cam_str
    )
    seg_dir = os.path.join(
        semseg_root, scene, variant, "frames", "classSegmentation", cam_str
    )
...
def process_scene(args):
    ...
    frames = discover_frames(
        args.rgb_root, args.depth_root, args.semseg_root, scene, variant, camera
    )
    if not frames:
    ...
    # Scene18/20: no terrain (class 1) — terrain labels are noisy in
    # these scenes
    ...
    for frame_id, rgb_path, depth_path, seg_path in tqdm(
        frames, desc=f"{scene}/{variant}"
    ):
        # Load RGB
    ...
    with h5py.File(h5_path, "w") as f:
        f.create_dataset(
            "rgb",
            data=np.stack(rgb_list),
            compression="gzip",
            compression_opts=4,
        )
        f.create_dataset(
            "depth",
            data=np.stack(depth_list),
            compression="gzip",
            compression_opts=4,
        )
        f.create_dataset(
            "semantic",
            data=np.stack(semseg_list),
            compression="gzip",
            compression_opts=4,
        )
        f.create_dataset(
            "planes",
            data=np.stack(planes_list),
            compression="gzip",
            compression_opts=4,
        )
    ...
    print(
        f"[DONE] {scene}/{variant}: {len(planes_list)} frames, "
...
def main():
    ...
    parser.add_argument(
        "--scene", type=str, required=True, help="Scene name (e.g., Scene01)"
    )
    parser.add_argument(
        "--variant",
        type=str,
    ...
    parser.add_argument(
        "--camera", type=int, default=0, help="Camera index (0=left, 1=right)"
    )
    parser.add_argument(
    ...
    for attr in ["rgb_root", "depth_root", "semseg_root", "output_root"]:
        if getattr(args, attr, None) is None:
            print(
                f"[ERROR] Missing required argument: --{attr} (or --data_root)"
            )
            sys.exit(1)
