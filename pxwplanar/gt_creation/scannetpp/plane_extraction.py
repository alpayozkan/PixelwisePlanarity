#!/usr/bin/env python3
"""
Ultra-fast planes-only pipeline + Policies + LAST-STAGE RELAXED REGION
GROWTH + Recursive Large-Label Split
...
Policies:
- --policy_single_plane_labels: Fit one plane for each listed label
  (raw name or id).
- --policy_skip_labels: Skip processing for each listed label
  (no planes for them).

Recursive large-label safeguard:
- If a (sub)label has >= --large_split_verts unique vertices and
  --large_split_enable=1, we recursively split its faces into two spatial
  halves (axis-variance or PCA median cut) until all parts are below
  threshold or we hit --large_split_max_parts or min-size guards.  Each
  part is processed independently; at the end we run a dedicated per-label
  merge
  across all parts to reunify compatible planes.

Final stage (relaxed normal gate + centroid distance) as region growing:
- For each plane A (within its label only), starting from A's faces, grow to
  adjacent faces satisfying |n·centroid + d| <= last_dist_m AND
  abs(FN·n) >= cos(last_normal_deg), and same semantic label. Unlabeled
  additions and stealing respect your given ratios.
...
# -------------------- segments → labels --------------------
def build_vertex_labels_from_segments(
    mesh: trimesh.Trimesh, seg_json_path: str, anno_json_path: str
) -> tuple[np.ndarray, dict[int, str]]:
    if not os.path.isfile(seg_json_path):
        sys.exit(f"[ERR] segments.json not found: {seg_json_path}")
    if not os.path.isfile(anno_json_path):
        sys.exit(f"[ERR] segments_anno.json not found: {anno_json_path}")
    with open(seg_json_path) as fh:
        segs = json.load(fh)
    seg_idx = np.asarray(segs["segIndices"])
    Vn = mesh.vertices.shape[0]
    Fn = mesh.faces.shape[0]

    with open(anno_json_path) as fh:
        anno = json.load(fh)["segGroups"]
    sid2raw = {}
    for g in anno:
    ...
    def ids_to_raw(arr):
        ...
        for i, sid in enumerate(arr):
            out[i] = sid2raw.get(int(sid), "unannotated")
        return out
    ...
    if seg_idx.shape[0] == Fn:
        ...
        for v in range(Vn):
            if buckets[v]:
                vals, cnts = np.unique(
                    np.array(buckets[v], np.int32), return_counts=True
                )
...
def sweep_inliers_label(
    ...
    Label-restricted sweep:
      ...
      - Distance gate options:
          "kof3"   : >=k of 3 triangle vertices within dist_m of plane
                     (k=ceil(3*frac_vertices)).
...
def saturate_within_label(
    ...
    for _ in range(max(1, rounds)):
        ...
        for p in patches_label:
            ...
            union = np.unique(np.concatenate([p["faces"], sweep])).astype(
                np.int32
            )
...
def build_label_stats(
    labels_f: np.ndarray, F: np.ndarray, FA: np.ndarray, int2raw: dict[int, str]
):
    """
    Precompute simple stats per semantic label:
      - faces (triangles), unique vertex count, total area, raw name.
    Returns: dict[label_id] ->
        {"faces": int, "verts": int, "area": float, "raw": str}
    """
    stats = {}
...
def split_label_faces_into_two(
    ...
    """Split faces_in_label into two halves by spatial median
    (axis-variance or PCA)."""
...
def last_stage_relaxed_rg(
    ...
    return rebuild_planes_from_face_pid(
        face_pid, F, V, FA, labels_f, int2raw, irls_max_iters, irls_eps
    )
...
def main():
    ap = argparse.ArgumentParser(
        "Planes-only Label-STRICT RG + EM + Saturation + Policies + "
        "Last-stage RG + Recursive Large-Split"
    )
    ...
    ap.add_argument(
        "--mesh",
        required=True,
        help="ScanNet-style semantic mesh (PLY/OBJ/GLB supported by trimesh)",
    )
    ap.add_argument(
        "--segments_json", required=True, help="ScanNet segments.json"
    )
    ap.add_argument(
        "--segments_anno", required=True, help="ScanNet segments_anno.json"
    )
    ...
    ap.add_argument(
        ...
        default="",
        help=(
            "Comma/semicolon-separated list of label ids or raw names to "
            "assign as single planes."
        ),
    )
    ap.add_argument(
        "--policy_skip_labels",
    ...
    ap.add_argument(
        ...
        default=1,
        help=(
            "If 1, labels with >= --large_split_verts unique verts are "
            "(recursively) split."
        ),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        help=(
            "Parallel backend "
            "(threads default; processes may speed Python loops)"
        ),
    )
    ...
    ap.add_argument(
        "--thickness_max_mul", type=float, default=1.6
    )  # * sweep_dist_m
    ...
    ap.add_argument(
        "--thickness_max_small_mul", type=float, default=2.2
    )  # * sweep_dist_m_small
    ...
    ap.add_argument(
        "--last_enable", type=int, default=1, help="Enable final growth stage"
    )
    ap.add_argument(
        "--last_dist_m",
        type=float,
    ...
    ap.add_argument(
        ...
        default=18.0,
        help=(
            "Relaxed normal gate for last-stage (deg); "
            "larger is more permissive"
        ),
    )
    ap.add_argument(
        ...
        default=1.0,
        help=(
            "Max ratio (added_component_area / A_base_area) for unlabeled "
            "components"
        ),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        default=1,
        help=(
            "Iterations of relaxed region growing per plane (1 is recommended)"
        ),
    )
    ...
    if not isinstance(mesh, trimesh.Trimesh):
        sys.exit("[ERR] mesh is not a single Trimesh.")
    ...
    labels_v, int2raw = build_vertex_labels_from_segments(
        mesh, args.segments_json, args.segments_anno
    )
    ...
    if single_plane_set:
        ...
        for lbl in bar_policy:
            ...
            pl = fit_plane_irls(
                V[verts], max_iters=args.irls_max_iters, eps=args.irls_eps
            )
            if pl is None:
                PWRITE(
            ...
            PWRITE(
                f"[POLICY] {raw} (id={lbl})  F={faces_in_label.size:,}  "
                f"V={verts.size:,}  area={area:.3f}"
            )
    ...
    for lbl in process_labels:
        ...
        verts_count = label_stats.get(int(lbl), {}).get(
            "verts", int(np.unique(F[faces_in_label].reshape(-1)).size)
        )
        info = {
            "faces": int(faces_in_label.size),
            "verts": int(verts_count),
            "area": float(FA[faces_in_label].sum()),
            "raw": raw,
        }
    ...
    if args.jobs > 1 and len(work_items) > 1:
        with Executor(max_workers=args.jobs) as ex:
            ...
            pbar = TQDM(
                total=len(futures),
                desc=f"Labels (pass A, {args.backend})",
                disable=not show_bar,
                dynamic_ncols=True,
            )
        ...
        for lbl in pbar_rec:
            ...
            for reg in regs_sorted:
                ...
                for _ in range(max(1, args.em_max_iters_small)):
                    ...
                    union = np.unique(np.concatenate([curr, sweep])).astype(
                        np.int32
                    )
                    ok2, plane2, _ = fit_and_quality(
                        union,
    ...
    if len(planes_meta) == 0:
        print(
            ...
            "consider relaxing parameters or gate_mode."
        )
