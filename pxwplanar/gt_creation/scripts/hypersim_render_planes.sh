...
INPUT_ROOT="${2:-$(python -c "import sys; sys.path.insert(0, '$REPO_ROOT'); from pxwplanar.paths import hypersim_params_path; print(hypersim_params_path)")}"
PLANE_ROOT="${3:-$(python -c "import sys; sys.path.insert(0, '$REPO_ROOT'); from pxwplanar.paths import hypersim_plane_path; print(hypersim_plane_path)")}"
OUTPUT_ROOT="${4:-$(python -c "import sys; sys.path.insert(0, '$REPO_ROOT'); from pxwplanar.paths import hypersim_rendered_path; print(hypersim_rendered_path)")}"
FRAME_SKIP="${5:-1}"
PYTHON_SCRIPT="${6:-$SCRIPT_DIR/../hypersim/rendering.py}"
METADATA_CSV="${7:-$REPO_ROOT/pxwplanar/shared/datasets/metadata_camera_parameters.csv}"

# Activate conda environment
source activate pxwplanar 2>/dev/null || conda activate pxwplanar 2>/dev/null || true
