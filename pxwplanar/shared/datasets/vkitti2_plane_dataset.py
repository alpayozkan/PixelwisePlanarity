...
Data format:
    data_root/
        <scene>/              (Scene01, Scene02, Scene06, Scene18, Scene20)
            <variant>/        (clone, fog, morning, overcast, rain, sunset,
                               15-deg-left, ...)
                scene_data.h5
...
Each scene_data.h5 contains:
    ...
    c2w:        (N, 4, 4)         float32
    frame_ids:  (N,)              bytes
    Attributes: fx, fy, cx, cy, image_height, image_width, scene, variant,
                num_frames
"""
...
class VKITTI2PlaneDataset(Dataset):
    ...
    Args:
        ...
        split_txt_dir: Directory containing split files (train.txt,
            val.txt, test.txt). Each line is a scene name (e.g.,
            "Scene01"). All variants of that scene are included unless
            filtered by `variants`.
        split: 'train', 'val', or 'test'.
        variants: List of variant names to include. None = all variants
            found on disk.
    ...
    def __init__(
        ...
        # Build valid pairs
        # (h5_path, frame_idx_int, scene, variant, K, frame_id_str)
        self.valid_pairs = []
        valid_scene_ids = []
        ...
        for scene in scene_ids:
            ...
            if variants is not None:
                available_variants = [
                    v for v in available_variants if v in variants
                ]
            ...
            for variant in available_variants:
                ...
                for idx in range(num_frames):
                    ...
                    self.valid_pairs.append(
                        (h5_path, idx, scene, variant, K, fid)
                    )
                    scene_frame_count += 1
        ...
        print(
            f"[VKITTI2] {split} split → {len(self.valid_pairs)} pairs "
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(h5_path, "r") as f:
            ...
            image = torch.tensor(rgb.astype(np.float32) / 255.0).permute(
                2, 0, 1
            )  # (3, H, W)
        ...
        if target_h != H or target_w != W:
            ...
            img_np = cv2.resize(
                img_np, (target_w, target_h), interpolation=cv2.INTER_LINEAR
            )
