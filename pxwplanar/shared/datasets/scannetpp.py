...
class ScanNetPPPlaneDataset(Dataset):
    def __init__(
        ...
        # require_gt=False: skip the rendered plane/sem/depth H5 existence
        # checks and read frame IDs from the pose JSON instead of from
        # rendered.h5. Use this for inference-only runs where ground truth
        # is not available. __getitem__ already handles missing GT via
        # try/except and zero-fill.
        ...
        #     split_txt_dir, f"nvs_sem_{split}_with_planes.txt"
        # )
        ...
        # (rgb_path, plane_h5, sem_h5, depth_h5, frame_idx, K)
        self.valid_pairs = []
        ...
        for scene_id in scene_ids:
            ...
            #     plane_label_root, scene_id, "rendered_planes.h5"
            # )
            ...
            depth_h5 = os.path.join(
                depth_label_root, scene_id, "rendered_depth.h5"
            )
            ...
            if require_gt and not (
                os.path.exists(plane_h5) and os.path.exists(depth_h5)
            ):
            ...
            # Frame IDs: prefer rendered.h5 ordering; fall back to pose
            # JSON keys
            frame_ids = None
            if os.path.exists(plane_h5):
                try:
                    with h5py.File(plane_h5, "r") as f:
                        frame_ids = [
                            fid.decode("utf-8") for fid in f["frame_ids"][:]
                        ]
                except Exception as e:
                    print(
                        f"[WARN] Error reading frame_ids from {plane_h5}: {e}"
                    )
            if frame_ids is None:
                if require_gt:
                    print(
                        f"[SKIP] Could not read frame_ids and "
                        f"require_gt=True for {scene_id}"
                    )
                    continue
                frame_ids = natsorted(pose_data.keys())
                print(
                    f"[INFO] Using {len(frame_ids)} frame_ids from pose "
                    f"JSON for {scene_id}"
                )
            ...
            if (
                ...
                try:
                    ...
                    if depth_fids != frame_ids:
                        print(
                            f"[SKIP] {scene_id}: rendered_depth.h5 "
                            "frame_ids do not match rendered.h5 "
                            f"({len(depth_fids)} vs {len(frame_ids)} "
                            "frames) — re-render with the same frame_skip"
                        )
                        continue
                except KeyError:
                    # legacy depth H5 without frame_ids: keep positional
                    # indexing
                    pass
            ...
            for idx, fid in enumerate(frame_ids):
                ...
                K = np.array(
                    pose_data[fid]["intrinsic"], dtype=np.float32
                )  # (3, 3)
                c2w = np.array(
                    pose_data[fid]["aligned_pose"], dtype=np.float32
                )  # (4, 4)
                self.valid_pairs.append(
                    (rgb_path, plane_h5, sem_h5, depth_h5, idx, K, c2w)
                )
                scene_frame_count += 1
            ...
            if scene_frame_count > 0:
                ...
                print(
                    f"[DEBUG] Scene {scene_id} → "
                    f"{scene_frame_count} matched frames"
                )

        # Update scene_ids to only include valid scenes
        self.scene_ids = valid_scene_ids
        print(
            f"[ScanNet++] {split} split → {len(self.valid_pairs)} pairs "
    ...
    def __getitem__(self, idx):
        rgb_path, plane_h5, sem_h5, depth_h5, frame_idx, K, c2w = (
            self.valid_pairs[idx]
        )

        if not self.require_gt:
            # Inference-only: skip GT reads entirely (avoids per-frame
            # [WARN] spam and h5py overhead).
            ...
            try:
                ...
                print(
                    f"[WARN] Failed plane label from {plane_h5} "
                    f"[{frame_idx}]: {e}"
                )
                H, W = self.image_height, self.image_width
                plane = torch.zeros((1, H, W), dtype=torch.float32)
            ...
            # --- Semantic label
            #     (optional; zero-filled when the file is absent) ---
            if os.path.exists(sem_h5):
                try:
                    with h5py.File(sem_h5, "r") as f:
                        sem = f["sem"][frame_idx]
                    sem = torch.from_numpy(sem.astype(np.int64)).unsqueeze(
                        0
                    )  # [1, H, W]
                except Exception as e:
                    print(
                        f"[WARN] Failed semantic label from {sem_h5} "
                        f"[{frame_idx}]: {e}"
                    )
                    sem = torch.zeros((1, H, W), dtype=torch.int64)
            else:
        ...
        image = torch.tensor(image / 255.0, dtype=torch.float32).permute(
            2, 0, 1
        )  # [3, H, W]

        # return image, depth, plane, sem, rgb_path, torch.from_numpy(K)
        # return (image, depth, plane, sem, rgb_path,
        #         torch.from_numpy(K), torch.from_numpy(c2w))
