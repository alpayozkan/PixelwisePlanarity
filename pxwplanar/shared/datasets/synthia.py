...
class SYNTHIAPlanarityDataset(Dataset):
    ...
    Args:
        data_root: Path to synthia/train or synthia/test (raw data, used
            to discover scene names)
        plane_label_root: Root of pre-computed H5 files (scene_data.h5 per
            scene)
    ...
    def __init__(
        ...
        for scene_name in scene_names:
            h5_path = os.path.join(
                plane_label_root, scene_name, "scene_data.h5"
            )
            ...
            try:
                with h5py.File(h5_path, "r") as hf:
                    ...
                    frame_ids = [
                        fid.decode("utf-8") for fid in hf["frame_ids"][:]
                    ]
        ...
        if max_samples is not None and len(self.valid_pairs) > max_samples:
            indices = np.linspace(
                0, len(self.valid_pairs) - 1, max_samples, dtype=int
            )
            self.valid_pairs = [self.valid_pairs[i] for i in indices]

        print(
            f"[SYNTHIA] {split} split -> {len(self.valid_pairs)} pairs "
            f"from {len(scene_names)} scenes"
        )
    ...
    def __getitem__(self, idx):
        ...
        semantic = cv2.resize(
            semantic.astype(np.float32), (W, H), interpolation=cv2.INTER_NEAREST
        ).astype(np.int64)
        plane = cv2.resize(
            plane.astype(np.float32), (W, H), interpolation=cv2.INTER_NEAREST
        )

        # Convert to tensors
        image = torch.tensor(rgb / 255.0, dtype=torch.float32).permute(
            2, 0, 1
        )  # (3, H, W)
        depth = torch.tensor(depth.copy(), dtype=torch.float32).unsqueeze(
            0
        )  # (1, H, W)
        semantic = torch.tensor(semantic.copy(), dtype=torch.int64).unsqueeze(
            0
        )  # (1, H, W)
        plane = torch.tensor(
            (plane > 0).astype(np.float32), dtype=torch.float32
        ).unsqueeze(0)  # (1, H, W)
        ...
        return {
            ...
            if entry["frame_id"].isdigit()
            else hash(entry["frame_id"]) % 100000,
        }
