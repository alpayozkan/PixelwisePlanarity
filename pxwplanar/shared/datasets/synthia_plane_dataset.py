...
Dataset structure:
    data_root/
        train/
            <scene_name>/
                scene_data.h5   (rgb, depth, planes, semantic, frame_ids + root attrs with K)
                planes.json     (per-plane metadata: normals, distances, class names, p95)
        test/
            <scene_name>/
                ...
...
planes.json:
    List of dicts with keys: frame_id, plane_id, n (3-vec), d, num_pixels, p95, class_id, class_name
...
class SynthiaPlaneDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            split: 'train' or 'test'.
            image_height: Target image height for resizing (default 480 = native).
        ...
        for scene_name in scene_names:
            ...
            for idx, fid in enumerate(frame_ids):
                self.valid_pairs.append((scene_name, h5_path, json_path, idx, fid, K))

            valid_scene_ids.append(scene_name)

        self.scene_ids = valid_scene_ids
        print(f"[Synthia] {split} split → {len(self.valid_pairs)} frames "
    ...
    def __getitem__(self, idx):
        scene_name, h5_path, json_path, frame_idx, frame_id, K = self.valid_pairs[idx]
        ...
        need_resize = (H != H_native) or (W != W_native)

        # --- RGB ---
        if need_resize:
            rgb_raw = cv2.resize(rgb_raw, (W, H), interpolation=cv2.INTER_LINEAR)
        image = torch.from_numpy(rgb_raw.astype(np.float32) / 255.0).permute(2, 0, 1)  # (3, H, W)

        # --- Depth ---
        if need_resize:
            depth_raw = cv2.resize(depth_raw, (W, H), interpolation=cv2.INTER_LINEAR)
        depth = torch.from_numpy(depth_raw.astype(np.float32)).unsqueeze(0)  # (1, H, W)

        # --- Plane labels ---
        if need_resize:
            plane_raw = cv2.resize(plane_raw, (W, H), interpolation=cv2.INTER_NEAREST)
        plane = torch.from_numpy(plane_raw.astype(np.int32)).unsqueeze(0)  # (1, H, W)

        # --- Semantic labels ---
        if need_resize:
            sem_raw = cv2.resize(sem_raw, (W, H), interpolation=cv2.INTER_NEAREST)
        sem = torch.from_numpy(sem_raw.astype(np.int64)).unsqueeze(0)  # (1, H, W)
        ...
        return {
            ...
            "rgb_path": f"{scene_name}/{frame_id}", # virtual path (no file on disk)
            "scene_id": scene_name,
            "frame_idx": frame_id,
        }
