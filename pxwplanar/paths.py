...
#   1. set the PXWPLANAR_DATA_ROOT environment variable
#      (default: <repo>/data), or
#   2. create pxwplanar/paths_local.py (gitignored) redefining any
#      variables below.
...
data_root = os.environ.get(
    "PXWPLANAR_DATA_ROOT", os.path.join(repo_path, "data")
)
...
scannetpp_rend_plane_path = os.path.join(
    data_root, "plane_gt_rendered", "scannetpp"
)
...
# ============ Models / checkpoints ============
# Trained MoGe 4-head planarity checkpoint
# (HIRES, 4 datasets, epoch 1 — production)
planarity_model_path = os.path.join(
    data_root, "checkpoints", "moge_HIRES_4datasets", "model_epoch1.pt"
)
...
# Optional site-specific overrides (create pxwplanar/paths_local.py;
# gitignored).
