...
#
# Files under MoGe/ are never touched: it is a git submodule, so `git ls-tree`
# reports it as a single entry rather than as Python files, and ruff.toml
# excludes it for good measure.

# Check version
version_string=$(ruff --version | awk '{print $NF}')
...
if [[ "$version_string" =~ "$expected_version_string" ]]; then
    ...
    echo "install it with: pip install ruff==${expected_version_string}"
    exit 1
fi
