...
- Methods with a different non-planar label (e.g. ZeroPlane uses 20) are remapped via nonplanar_label
...
Usage:
    ...
    python evaluate_all_baselines.py --methods ours       # Evaluate specific methods
...
# - h5_folder: per-scene planes.h5 folder under H5_ROOT (paths.inference_h5_root);
#   use h5_root_override instead for an absolute root.
# - nonplanar_label: label meaning "non-planar" in a method's H5s (remapped to 0);
...
METHODS = {
    ...
    "ours": {
        # planes.h5 produced by the release pipeline (save_moge_signals_planarity.py
        # -> segment_signals_to_planes.py) with the 4-head MoGe checkpoint
        # (moge_HIRES_4datasets, epoch 1) at 1440x1920, canonical segmentation
