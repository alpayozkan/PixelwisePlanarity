...
This script reads the full scene lists and creates standard train/val/test
splits
...
def create_splits(
    ...
    for split_name, split_scenes in splits.items():
        ...
        with open(output_file, "w") as f:
            f.write("\n".join(split_scenes) + "\n")
        print(
            f"[{split_name:5s}] {len(split_scenes):4d} scenes -> {output_file}"
        )

    print(f"\nTotal: {n_total} scenes")
    print(
        f"  Train: {len(train_scenes)} "
        f"({len(train_scenes) / n_total * 100:.1f}%)"
    )
    print(
        f"  Val:   {len(val_scenes)} ({len(val_scenes) / n_total * 100:.1f}%)"
    )
    print(
        f"  Test:  {len(test_scenes)} ({len(test_scenes) / n_total * 100:.1f}%)"
    )
