...
DISTANCE_THRESHOLD = 0.01  # 1cm threshold for inliers
INLIER_RATIO_THRESHOLD = 0.5  # Quality gate for plane fitting
...
# Method definitions matching evaluate_all_baselines.py
METHODS = {
    "GT": {
        "h5_folder": None,  # Uses GT from dataset
        "display_name": "GT",
        "label_offset": 0,
    ...
    "ours": {
        ...
        "display_name": "MoGe-Ours",
        "label_offset": 0,
        "uses_rendered_h5": False,
    },
    "zeroplane": {
    ...
    "gtplanarity_ourseg": {
        ...
        "display_name": "GT Plan + Our Seg",
        "label_offset": 0,
        "uses_rendered_h5": False,
    },
    "ourplanarity_gtseg": {
        "h5_folder": "ourplanarity_gtseg_v1_h5",
        "display_name": "Our Plan + GT Seg",
        "label_offset": 0,
        "uses_rendered_h5": False,
    },
}
...
def visualize_frame(
    ...
    plt.suptitle(f"RANSAC Inliers @ {DISTANCE_THRESHOLD*100:.0f}cm | {scene_id} / {frame_idx}\n{legend_text}", fontsize=12)
    plt.tight_layout()
...
def main():
    ...
    if args.output_dir:
        ...
        output_dir = VIS_ROOT / f"baselines_n{args.n_samples}_seed{args.random_seed}"
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"[CONFIG] N samples: {args.n_samples}")
    print(f"[CONFIG] Random seed: {args.random_seed}")
    print(f"[CONFIG] Output dir: {output_dir}")
    print(f"[CONFIG] Distance threshold: {DISTANCE_THRESHOLD*100:.0f}cm")
    ...
    print(f"\n==> Visualizing {len(frame_specs)} frames")
    for scene_id, frame_idx in tqdm(frame_specs, desc="Visualizing"):
        print(f"\n[Frame] {scene_id} / {frame_idx}")

        # Load sample
        key = (scene_id, frame_idx)
        if key not in frame_index:
            print(f"  [SKIP] Frame not found in dataset")
            continue
    ...
    print("\n==> Saving results")
    if all_results:
        save_results_h5(all_results, output_dir / "results.h5")
        save_summary_csv(all_results, output_dir / "summary.csv")

    print(f"\n[DONE] Visualized {len(all_results)} frames")
