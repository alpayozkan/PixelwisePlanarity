#!/bin/bash
# Submit v10 grid search jobs to SLURM — both affine and metric depth.
#
...
#
# Usage:
#   ./submit_grid_search_v10.sh                          # both pipelines, 15 eval jobs each
#   ./submit_grid_search_v10.sh --num-eval-jobs 10       # 10 eval jobs each
#   ./submit_grid_search_v10.sh --only affine            # only affine
#   ./submit_grid_search_v10.sh --only metric            # only metric
#   ./submit_grid_search_v10.sh --eval-only              # skip cache for both
...
")
    ...
    if [ "$EVAL_ONLY" = false ]; then
        ...
        CACHE_JOB_ID=$(sbatch \
            ...
            --mem-per-cpu=8G \
            --gpus=rtx_3090:1 \
            --wrap="bash -c '${ENV_ACTIVATE} && cd ${SCRIPT_DIR} && python grid_search_v10.py --yaml ${YAML_PATH} --mode cache'" \
            --parsable)
    ...
    for RANGE in $JOB_RANGES; do
        ...
        local JOB_ID
        JOB_ID=$(sbatch \
            --job-name="gsv10_${TAG}_eval_${JOB_IDX}" \
            ...
            --mem-per-cpu=10G \
            ${DEPENDENCY_FLAG} \
            --wrap="bash -c '${ENV_ACTIVATE} && cd ${SCRIPT_DIR} && python grid_search_v10.py --yaml ${YAML_PATH} --mode eval --config-start ${CFG_START} --config-end ${CFG_END}'" \
            --parsable)
    ...
    AGG_JOB_ID=$(sbatch \
        ...
        --wrap="bash -c '${ENV_ACTIVATE} && cd ${SCRIPT_DIR} && python grid_search_v10.py --yaml ${YAML_PATH} --mode aggregate'" \
        --parsable)
...
if [ "$ONLY_MODE" != "metric" ]; then
    submit_pipeline "${SCRIPT_DIR}/grid_search_v10_config.yaml" "affine"
fi

if [ "$ONLY_MODE" != "affine" ]; then
    submit_pipeline "${SCRIPT_DIR}/grid_search_v10_config_metric.yaml" "metric"
fi

echo ""
echo "============================================"
echo "Monitor: squeue -u \$USER | grep gsv10_"
echo "============================================"
