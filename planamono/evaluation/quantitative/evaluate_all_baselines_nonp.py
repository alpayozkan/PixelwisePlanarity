"""
Unified evaluation script for all baseline methods (with non-planar region handling).

This version properly handles ZeroPlane's label 20 as non-planar regions (mapped to 0).
...
Usage:
    python evaluate_all_baselines_nonp.py                    # Evaluate all methods
    python evaluate_all_baselines_nonp.py --methods ours zeroplane  # Evaluate specific methods
    python evaluate_all_baselines_nonp.py --aggregate-only   # Only aggregate existing results
"""
...
EXP_VER = "v4_nonp"  # Updated version with non-planar handling
