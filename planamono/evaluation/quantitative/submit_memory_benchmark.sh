#!/bin/bash
# Submit memory_benchmark_region_growing.py jobs across N shards on
# scannetpp scenes from MoGe signals, then chain a single aggregator
# behind all of them.
#
# Mirrors submit_runtime_benchmark.sh — same shard / aggregator pattern,
# same GPU resource model (the kernel is torch.cuda inside
# compute_vectorized_planar_segments_v5_relative). Only the python target
# script and the output root differ:
#     PY  = memory_benchmark_region_growing.py
#     OUT = .../eval/moge_memory_benchmark/...
#
# Defaults:
#   EXP                 = "memory_v5rel"
...
#
# Usage:
#   bash submit_memory_benchmark.sh
#   bash submit_memory_benchmark.sh --exp my_run --parts 4 --frames 50
#   bash submit_memory_benchmark.sh --device cpu          # CPU-only mode
#                                                         # (uses tracemalloc
#                                                         #  inside the python
#                                                         #  script)
#
# Output:
#   /cluster/scratch/aoezkan/planeseg/eval/moge_memory_benchmark/
...
EVAL_ROOT="${EVAL_ROOT:-/cluster/scratch/aoezkan/planeseg/eval/moge_memory_benchmark}"
...
PY_SCRIPT="$PROJECT_ROOT/evaluation/quantitative/memory_benchmark_region_growing.py"

LOG_DIR="/cluster/scratch/aoezkan/planeseg/logs/memory_benchmark/$EXP"
mkdir -p "$LOG_DIR"
...
echo "================================================================"
echo " memory_benchmark_region_growing — multi-shard submission"
echo "================================================================"
echo " EXP:               $EXP"
...
echo "==== memory benchmark — part $PART/$PARTS ($NUM_S scenes) ===="
