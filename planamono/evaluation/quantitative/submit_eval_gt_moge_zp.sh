#!/bin/bash
# Submit evaluate_gt_moge_zeroplane.py jobs across (method, dataset) cells with
# per-dataset scene partitioning, then chain a single aggregator job behind all
# of them.
#
...
#   bash submit_eval_gt_moge_zp.sh
#   bash submit_eval_gt_moge_zp.sh --methods moge --scenes 5 --frames 10 --n-jobs 4
#   bash submit_eval_gt_moge_zp.sh --exp my_exp --datasets scannetpp
#   METHODS=moge MAX_SCENES=5 MAX_FRAMES=10 bash submit_eval_gt_moge_zp.sh
#
# CLI flags:
#   --exp NAME              Experiment name (default: gt_moge_zp_v1).
...
PY_SCRIPT="$PROJECT_ROOT/evaluation/quantitative/evaluate_gt_moge_zeroplane.py"
LOG_DIR="/cluster/scratch/aoezkan/planeseg/logs/eval_gt_moge_zp/$EXP"
mkdir -p "$LOG_DIR"
...
echo "================================================================"
echo " evaluate_gt_moge_zeroplane — multi-(method, dataset) submission"
echo "================================================================"
echo " EXP:               $EXP"
    ...
    for DS in $DATASETS; do
        ...
        for PART in $(seq 0 $((N_PARTS_DS - 1))); do
            ...
            JOB_ID=$(/cluster/apps/slurm/bin/sbatch --parsable \
                ...
                --job-name="ev_${METHOD}_${DS}_p${PART}" \
