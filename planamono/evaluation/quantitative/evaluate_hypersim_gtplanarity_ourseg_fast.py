"""
Evaluation script for: GT Planarity + Our Segmentation ablation on Hypersim.
...
exp_name = f'hypersim_gtplanarity_ourseg_{EXP_TAG}_{EXP_VER}'
...
def process_batch_inference(
    ...
    with timer("_postprocess"):
        for res, rgb_path, scene_id, frame_id, gt_plane, depth_gt in zip(
            ...
            # Get MoGe depth and normals
            depth_moge = res["points"][:, :, 2]
            normal = res["normal"].transpose(2, 0, 1)
            H_moge, W_moge = depth_moge.shape

            # Create GT planarity mask (binary from GT plane labels)
            planarity = (gt_seg_np > 0).astype(np.int16)

            # Resize MoGe outputs to depth resolution for segmentation
            depth_moge_eval = cv2.resize(depth_moge, (W_depth, H_depth), interpolation=cv2.INTER_LINEAR)
            normal_eval = cv2.resize(
                normal.transpose(1, 2, 0), (W_depth, H_depth), interpolation=cv2.INTER_LINEAR
            ).transpose(2, 0, 1)

            # Run our segmentation algorithm with GT planarity + MoGe depth/normals
    ...
    with timer("streaming_pipeline"):
        for batch in tqdm(val_loader, desc="Processing"):
            ...
            batch_data = process_batch_inference(
                images_np, rgb_paths, scene_ids, frame_ids, gt_planes, depths,
