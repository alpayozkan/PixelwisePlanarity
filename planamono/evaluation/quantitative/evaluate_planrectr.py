#!/usr/bin/env python3
"""
Unified evaluation of PlanRecTR predictions across 4 datasets.

PlanRecTR H5 format (identical across all datasets):
...
Frame matching:
    ...
    Hypersim:   index-based — PlanRecTR index i → GT index i (per scene+camera)
    Synthia:    index-based — PlanRecTR index i → GT index i
    VKITTI2:    index-based — PlanRecTR index i → GT index i

Usage:
    python evaluate_planrectr.py --resolution lowres              # All datasets, lowres
    python evaluate_planrectr.py --resolution highres             # All datasets, highres
    python evaluate_planrectr.py --datasets scannetpp hypersim    # Specific datasets
    python evaluate_planrectr.py --max-scenes 1                   # Quick test
    python evaluate_planrectr.py --aggregate-only                 # Re-aggregate
"""
...
NONPLANAR_LABEL = 20  # PlanRecTR uses 20 for non-planar (same as ZeroPlane)
...
class PlanRecTRH5Loader:
    """Load PlanRecTR predictions from a single H5 file.
...
def _remap_planrectr_labels(seg: np.ndarray) -> np.ndarray:
    """Remap PlanRecTR segmentation to standard convention.

    PlanRecTR: 20 = non-planar, 1-19 = plane instances, 0 = void/unlabeled.
...
def _get_dataset_configs(planrectr_root: str) -> Dict:
    ...
    return {
        ...
        "hypersim": {
            "planrectr_root": os.path.join(planrectr_root, "hypersim"),
        ...
        "synthia": {
            "planrectr_root": os.path.join(planrectr_root, "synthia"),
        ...
        "vkitti2": {
            "planrectr_root": os.path.join(planrectr_root, "vkitti2"),
...
# ============================================================
# PlanRecTR H5 DISCOVERY
# ============================================================

def _discover_planrectr_scannetpp(planrectr_root: str) -> Dict[str, PlanRecTRH5Loader]:
    """Discover PlanRecTR H5 files for ScanNet++.

    Layout: <root>/<scene_id>/rendered_v2.h5
    Returns {scene_id: loader}.
    """
    loaders = {}
    root = Path(planrectr_root)
    if not root.exists():
        print(f"[ERROR] PlanRecTR root does not exist: {root}")
        return loaders
    ...
    for scene_dir in sorted(root.iterdir()):
        ...
        if h5_path.exists():
            loaders[scene_dir.name] = PlanRecTRH5Loader(str(h5_path))

    print(f"[PlanRecTR] Found {len(loaders)} ScanNet++ scenes in {planrectr_root}")
    return loaders


def _discover_planrectr_hypersim(planrectr_root: str) -> Dict[Tuple[str, str], PlanRecTRH5Loader]:
    """Discover PlanRecTR H5 files for Hypersim.
    ...
    for scene_dir in sorted(root.iterdir()):
        ...
        for h5_file in sorted(scene_dir.glob("rendered_planes_cam_*.h5")):
            ...
            loaders[(scene_dir.name, cam_name)] = PlanRecTRH5Loader(str(h5_file))

    print(f"[PlanRecTR] Found {len(loaders)} Hypersim scene+camera files in {planrectr_root}")
    return loaders


def _discover_planrectr_synthia(planrectr_root: str) -> Dict[str, PlanRecTRH5Loader]:
    """Discover PlanRecTR H5 files for Synthia.
    ...
    print(f"[PlanRecTR] Found {len(loaders)} Synthia scenes in {planrectr_root}")
    return loaders


def _discover_planrectr_vkitti2(planrectr_root: str) -> Dict[str, PlanRecTRH5Loader]:
    """Discover PlanRecTR H5 files for VKITTI2.
    ...
    for scene_dir in sorted(root.iterdir()):
        ...
        for variant_dir in sorted(scene_dir.iterdir()):
            ...
            if h5_path.exists():
                ...
                loaders[key] = PlanRecTRH5Loader(str(h5_path))

    print(f"[PlanRecTR] Found {len(loaders)} VKITTI2 scene/variant combos in {planrectr_root}")
    return loaders
...
def evaluate_scannetpp(cfg: Dict, max_scenes: int = None,
    ...
    """Evaluate PlanRecTR on ScanNet++ (name-based frame matching)."""
    ...
    scene_loaders = _discover_planrectr_scannetpp(cfg["planrectr_root"])
    ...
    for ds_idx in range(len(dataset)):
        ...
        frame_idx_str = os.path.splitext(os.path.basename(rgb_path))[0]
    ...
    for scene_id in sorted(scene_frames.keys()):
        ...
        for ds_idx, gt_frame_id in frames:
            ...
            labels = _remap_planrectr_labels(seg)
...
def evaluate_hypersim(cfg: Dict, max_scenes: int = None,
                      scene_start: int = None, scene_end: int = None):
    """Evaluate PlanRecTR on Hypersim (index-based, per scene+camera)."""
    kwargs = dict(cfg["dataset_kwargs"])
    if max_scenes is not None:
        kwargs["max_scenes"] = max_scenes
    cfg_copy = dict(cfg)
    ...
    cam_loaders = _discover_planrectr_hypersim(cfg["planrectr_root"])
    ...
    print(f"[Hypersim] Found {len(cam_loaders)} scene+camera PlanRecTR files"
...
def evaluate_index_based(
    ...
    """Evaluate PlanRecTR on Synthia / VKITTI2 (index-based matching)."""
    kwargs = dict(cfg["dataset_kwargs"])
    if max_scenes is not None:
        kwargs["max_scenes"] = max_scenes
    cfg_copy = dict(cfg)
    ...
    if dataset_key == "synthia":
        scene_loaders = _discover_planrectr_synthia(cfg["planrectr_root"])
    elif dataset_key == "vkitti2":
        scene_loaders = _discover_planrectr_vkitti2(cfg["planrectr_root"])
    else:
    ...
    for ds_idx in range(len(dataset)):
        if dataset_key == "vkitti2":
            ...
            sample_scene_id = dataset.valid_pairs[ds_idx][0]
...
def evaluate_dataset(dataset_key: str, cfg: Dict, exp_name: str,
                     max_scenes: int = None,
                     scene_start: int = None, scene_end: int = None):
    """Evaluate PlanRecTR on a single dataset."""
    display = cfg["display_name"]
    print(f"\n{'='*60}")
    print(f"Evaluating PlanRecTR on {display}")
    if scene_start is not None or scene_end is not None:
...
def aggregate_all(dataset_keys: List[str], configs: Dict, exp_name: str):
    """Print cross-dataset summary of PlanRecTR results."""
    print(f"\n{'='*80}")
    print(f"PLANRECTR CROSS-DATASET SUMMARY ({exp_name})")
    print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate PlanRecTR predictions across multiple datasets"
    )
    parser.add_argument(
        "--resolution", choices=["lowres", "highres"], required=True,
        help="PlanRecTR resolution variant to evaluate",
    )
    parser.add_argument(
    ...
    planrectr_root = PLANRECTR_ROOTS[args.resolution]
    exp_name = f"planrectr_{args.resolution}_{EXP_VER}"

    all_configs = _get_dataset_configs(planrectr_root)
    ...
    print(f"[CONFIG] PlanRecTR root: {planrectr_root}")
    ...
    print("\n[DONE] PlanRecTR evaluation complete!")
