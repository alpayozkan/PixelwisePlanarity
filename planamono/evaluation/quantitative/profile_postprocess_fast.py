"""
Profiling script to benchmark OPTIMIZED postprocessing functions.

Compare with profile_postprocess.py to measure speedup from:
1. compute_vectorized_planar_segments_v5 (batched Sobel + unfold)
2. remap_labels_fast (LUT vectorization)
3. resize_moge_outputs_fast (GPU batched resize)

Run both scripts and compare the timing output.
"""
...
from planamono.shared.utils.label_utils import remap_labels_fast
from planamono.shared.utils.utils import resize_moge_outputs_fast
...
class DetailedTimer:
    ...
    def print_summary(self):
        print("\n" + "="*70)
        print("PROFILING RESULTS (FAST VERSION)")
        print("="*70)
        ...
        print(f"{'Operation':<45} {'Total':>10} {'Count':>6} {'Avg':>10} {'%':>6}")
        print("-"*70)
        ...
        for name, t in sorted_items:
            ...
            print(f"{name:<45} {t:>9.2f}s {count:>6} {avg*1000:>9.1f}ms {pct:>5.1f}%")

        print("-"*70)
        print(f"{'TOTAL':<45} {total:>9.2f}s")


def process_batch_inference_profiled_fast(
    rgb_paths,
    scene_ids,
    frame_ids,
    gt_planes,
    ...
    """
    Profiled version of batch inference using OPTIMIZED functions.
    """
    # GPU inference
    ...
    for i, (res, rgb_path, scene_id, frame_id, gt_plane, depth_gt) in enumerate(zip(
        ...
        t0 = time.perf_counter()
        planarity = (gt_seg_np > 0).astype(np.float32)
        timer.add("planarity_mask", time.perf_counter() - t0)

        # 5. FAST: Resize MoGe outputs using GPU batched resize
        t0 = time.perf_counter()
        depth_moge, normal, planarity_rgb = resize_moge_outputs_fast(
            depth_moge, normal, planarity, H_rgb, W_rgb, device=str(args.device)
        )
        # normal is now (3, H, W), convert to (H, W, 3) for segmentation
        normal = normal.transpose(1, 2, 0)
        timer.add("resize_moge_fast", time.perf_counter() - t0)

        # 6. FAST: Segmentation using v5 (batched Sobel + unfold)
        t0 = time.perf_counter()
        labels_rgb, _ = compute_vectorized_planar_segments_v5(
            planarity_rgb.astype(bool),
            normal,
            depth_moge,
        ...
        timer.add("segmentation_v5", time.perf_counter() - t0)

        # 7. FAST: Remap labels using LUT
        t0 = time.perf_counter()
        labels_rgb, _ = remap_labels_fast(labels_rgb)
        timer.add("remap_labels_fast", time.perf_counter() - t0)
...
if __name__ == "__main__":
    ...
    print(f"[CONFIG] Using FAST optimized functions:")
    print(f"         - compute_vectorized_planar_segments_v5")
    print(f"         - remap_labels_fast")
    print(f"         - resize_moge_outputs_fast")
    ...
    print("==> Running profiled pipeline (FAST) on 1 scene")

    for batch in tqdm(val_loader, desc="Profiling (FAST)"):
        ...
        batch_data = process_batch_inference_profiled_fast(
            rgb_paths, scene_ids, frame_ids, gt_planes, depths,
            inference_model, args, timer
        )

    timer.print_summary()
    print(f"\n[DONE] Profiled {len(val_dataset)} frames (FAST version)")

    # Print comparison hint
    print("\n" + "="*70)
    print("To compare with original, run:")
    print("  python profile_postprocess.py")
    print("="*70)
