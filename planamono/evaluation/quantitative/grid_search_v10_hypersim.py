#!/usr/bin/env python3
"""
Grid search over v10 segmentation parameters on Hypersim val split.
...
Usage:
  python grid_search_v10_hypersim.py --yaml grid_search_v10_hypersim_config.yaml --mode cache
  python grid_search_v10_hypersim.py --yaml grid_search_v10_hypersim_config.yaml --mode eval --config-start 0 --config-end 3
  python grid_search_v10_hypersim.py --yaml grid_search_v10_hypersim_config.yaml --mode aggregate
"""
...
# ============================================================
# CONFIG GENERATION (copied from grid_search_v10.py)
# ============================================================
...
def run_cache(cfg: dict):
    """Run MoGe inference on Hypersim scenes and cache raw outputs."""
    ...
    BATCH_SIZE = 8  # Smaller batch for Hypersim (larger images)
    timer = Timer()
    ...
    for scene_id in selected_scenes:
        ...
        # Get M_cam_from_uv for this scene (needed for evaluation)
        M_cam_from_uv = dataset._get_M_cam_from_uv(scene_id)
        ...
        for batch_start in tqdm(
            ...
            for idx in batch_indices:
                ...
                # Convert image tensor (3,H,W) float[0,1] to uint8 numpy (H,W,3)
                img_tensor = sample["image"]  # (3, H, W) float [0,1]
                img_np = (img_tensor.permute(1, 2, 0).numpy() * 255).astype(np.uint8)
                images_np.append(img_np)

                cam_names.append(pair[1])  # cam_name from valid_pairs
                frame_idxs.append(sample["frame_idx"])
                gt_segs.append(sample["plane"])
                depths.append(sample["depth"])
                Ks.append(sample["K"])
                c2ws.append(sample["c2w"])
                native_whs.append(pair[-1])  # native_wh from valid_pairs

            # GPU inference (pass numpy arrays, preprocess_images handles them)
            ...
            with timer("extract"):
                for i, res in enumerate(results):
                    H_img, W_img = images_np[i].shape[:2]
                    ...
                    # Resize MoGe outputs to image resolution
                    planarity_rgb = cv2.resize(planarity, (W_img, H_img), interpolation=cv2.INTER_LINEAR)
                    depth_moge_rgb = cv2.resize(depth_moge, (W_img, H_img), interpolation=cv2.INTER_LINEAR)
                    normal_rgb = cv2.resize(normal, (W_img, H_img), interpolation=cv2.INTER_LINEAR)
                    ...
                    scene_frames.append({
                        ...
                        "native_wh": np.array(native_wh, dtype=np.int32),
                    })
        ...
        with timer("save_h5"):
            with h5py.File(h5_path, "w") as hf:
                ...
                hf.create_dataset("native_wh",  data=np.stack([f["native_wh"]  for f in scene_frames]))

                frame_idxs_enc = [f["frame_idx"].encode("utf-8") for f in scene_frames]
                hf.create_dataset("frame_idxs", data=frame_idxs_enc)

                cam_names_enc = [f["cam_name"].encode("utf-8") for f in scene_frames]
                hf.create_dataset("cam_names", data=cam_names_enc)

                hf.attrs["scene_id"] = scene_id

                # Store M_cam_from_uv as attribute (per-scene, needed for eval)
                if M_cam_from_uv is not None:
                    hf.attrs["M_cam_from_uv"] = M_cam_from_uv.astype(np.float64)
...
def run_eval_single(cfg: dict, config_index: int, configs: List[dict], scene_h5s: List[Path]):
    ...
    for h5_path in scene_h5s:
        with h5py.File(h5_path, "r") as hf:
            ...
            cam_names = [s.decode("utf-8") for s in hf["cam_names"][:]]

            # Load M_cam_from_uv (per-scene, stored as attr)
            M_cam_from_uv = None
            if "M_cam_from_uv" in hf.attrs:
                M_cam_from_uv = np.array(hf.attrs["M_cam_from_uv"])

            for fi in range(N):
                frame_idx = frame_idxs[fi]
                cam_name = cam_names[fi]
                full_frame_id = f"{cam_name}/{frame_idx}"
                ...
                K = hf["K"][fi]
                c2w = hf["c2w"][fi]
                native_wh = tuple(hf["native_wh"][fi])
                ...
                # Evaluate using Hypersim-specific function (backproject_mcam)
                with timer("evaluate"):
                ...
                results[(scene_id, full_frame_id)] = metrics
                pbar.update(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Grid search over v10 segmentation parameters (Hypersim)",
