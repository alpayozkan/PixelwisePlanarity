#!/usr/bin/env python3
"""
Unified evaluation script for all baseline methods on Parallel Domain dataset.
...
Uses standard pinhole backprojection (evaluate_single_frame) since PD has
proper camera intrinsics. Note: c2w is identity (no poses available in PD).

Usage:
    python evaluate_pd_all_baselines.py                    # Evaluate all methods
    python evaluate_pd_all_baselines.py --methods ours zeroplane  # Evaluate specific methods
    python evaluate_pd_all_baselines.py --aggregate-only   # Only aggregate existing results
"""
...
H5_ROOT = Path("/cluster/scratch/aoezkan/planeseg/pd/inference")
PD_ROOT = "/cluster/scratch/ayavuz/dataset/pd_zero/parallel_domain_plane"
...
# ============================================================
# H5 LOADING (one planes.h5 per scene_id)
# ============================================================

class LazyH5SceneLoader:
    """
    Memory-efficient loader that only keeps one scene in memory at a time.
    PD scene_id is extracted from origin_img_path (e.g., "scene_000168").
    """
...
def aggregate_results(methods: list, output_dir: Path = None):
    ...
    # Print summary
    print("\n" + "=" * 100)
    print("PARALLEL DOMAIN BASELINE RESULTS SUMMARY")
    print("=" * 100)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate all baseline methods on Parallel Domain")
