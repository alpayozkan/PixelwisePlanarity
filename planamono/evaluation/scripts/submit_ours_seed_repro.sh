#!/bin/bash
# Reproducibility sweep: evaluate OUR method (MoGe planarity + our segmentation)
# on ScanNet++ across multiple RANSAC seeds, each repeated several times.
#
# Mirror of submit_gt_seed_repro.sh, but for the "ours" method, which reads
...
#
#   ${OUTPUT_BASE}/ours_seed${SEED}_rep${IDX}/moge_ours_${EXP_VER}/
...
#
# Usage:
#   bash submit_ours_seed_repro.sh                 # 5 seeds x 2 repeats = 10 jobs
#   FORCE=1 bash submit_ours_seed_repro.sh         # re-run even if a run dir already has results
...
# Configuration
# ---------------------------------------------------------------------------
# Per-job bash script (handles conda activate + python; see run_ours_eval_job.sh).
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
JOB_SCRIPT="$SCRIPT_DIR/run_ours_eval_job.sh"
...
H5_FOLDER="moge_ours_v2_h5"          # ours -> H5_ROOT/moge_ours_v2_h5/<scene>/planes.h5
EXP_NAME_PREFIX="moge_ours"          # ours exp_name = "moge_ours_${EXP_VER}"
...
# Saved "ours" predictions (segmented planes.h5, 42/42 test scenes).
SRC_PRED_DIR="/cluster/scratch/ayavuz/scannetpp_backup2/planeseg/scannetpp/inference/moge_ours_h5"

# Base directory under which every (seed, repeat) run dir is created.
OUTPUT_BASE="/cluster/scratch/aoezkan/planeseg/scannetpp/eval/ours_seed_repro"
LOG_DIR="/cluster/scratch/aoezkan/planeseg/logs/eval_ours_repro"
...
echo "=== OURS reproducibility sweep ==="
echo "Method:       ours"
...
for SEED in "${SEEDS[@]}"; do
    for IDX in $(seq 1 "$REPEATS"); do
        RUN_NAME="ours_seed${SEED}_rep${IDX}"
...
echo "Monitor:  squeue -u \$USER --name=ours_seed*  ||  tail -f $LOG_DIR/ours_seed*_*.out"
