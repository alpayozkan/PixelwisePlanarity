#!/bin/bash
# Single OURS-evaluation job (one seed, one run dir). Submitted by
# submit_ours_seed_repro.sh as a real bash script so `source`/`conda activate`
# work (sbatch --wrap runs under /bin/sh, which breaks both).
#
# Usage (normally invoked via sbatch):
#   run_ours_eval_job.sh <SEED> <EVAL_ROOT> [NUM_WORKERS]
set -eu
...
echo "[SUCCESS] OURS eval done for seed $SEED -> $EVAL_ROOT"
