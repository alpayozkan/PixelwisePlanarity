#!/bin/bash
# Reproducibility sweep: evaluate METRIC3D on ScanNet++ across multiple RANSAC
# seeds, each repeated several times.
#
# Mirror of submit_zeroplane_seed_repro.sh, but for the "metric3d" method, which
# reads pre-computed (already segmented) per-scene rendered_v2.h5 (keys: planes,
# frame_ids; 0 = non-planar). Predictions are evaluated the SAME way as
# ours/zeroplane (GT depth + dataset K from ScanNetPPPlaneDataset; only the
# predicted `planes` are read), so the seed sweep is cross-method consistent.
#
# NOTE: this is NOT the same as the dedicated evaluate_metric3d.py, which uses
# the H5's own intrinsics (fx≈474, scaled to 480×640) and depth. Those numbers
# are not directly comparable to this sweep -- this sweep is for studying RANSAC
# seed variance under the SAME conditions as the ours/zeroplane sweeps.
#
# The eval script (evaluate_all_baselines.py) reads predictions from
#   ${H5_ROOT}/${H5_FOLDER}/<scene>/rendered_v2.h5
...
# is idempotent and refuses to overwrite a real directory. (The rendered_v2.h5
# filename is configured via the metric3d entry's "h5_filename" in METHODS.)
#
...
#
#   ${OUTPUT_BASE}/metric3d_seed${SEED}_rep${IDX}/metric3d_${EXP_VER}/
...
#
# Usage:
#   bash submit_metric3d_seed_repro.sh                 # 5 seeds x 2 repeats = 10 jobs
#   FORCE=1 bash submit_metric3d_seed_repro.sh         # re-run even if a run dir already has results
...
# Configuration
# ---------------------------------------------------------------------------
# Per-job bash script (handles conda activate + python; see run_metric3d_eval_job.sh).
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
JOB_SCRIPT="$SCRIPT_DIR/run_metric3d_eval_job.sh"
...
H5_FOLDER="metric3d_h5"              # metric3d -> H5_ROOT/metric3d_h5/<scene>/rendered_v2.h5
EXP_NAME_PREFIX="metric3d"           # metric3d exp_name = "metric3d_${EXP_VER}"
...
# Saved Metric3D predictions (per-scene rendered_v2.h5, 0 = non-planar, 42/42 test scenes).
SRC_PRED_DIR="/cluster/scratch/ayavuz/external_models/metric3d_scannetpp"

# Base directory under which every (seed, repeat) run dir is created.
OUTPUT_BASE="/cluster/scratch/aoezkan/planeseg/scannetpp/eval/metric3d_seed_repro"
LOG_DIR="/cluster/scratch/aoezkan/planeseg/logs/eval_metric3d_repro"
...
# SLURM resources (CPU-only: predictions are pre-computed, no GPU).
SLURM_TIME="4:00:00"
SLURM_CPUS=16
...
echo "=== METRIC3D reproducibility sweep ==="
echo "Method:       metric3d"
...
for SEED in "${SEEDS[@]}"; do
    for IDX in $(seq 1 "$REPEATS"); do
        RUN_NAME="metric3d_seed${SEED}_rep${IDX}"
...
echo "Monitor:  squeue -u \$USER --name=metric3d_seed*  ||  tail -f $LOG_DIR/metric3d_seed*_*.out"
