"""
Image-based LO-RANSAC plane extraction from depth maps.

Shared module used by VKITTI2 and SYNTHIA GT generation pipelines.
Extracts planes per semantic label using depth point clouds + surface normals.
"""
...
def local_optimize(n, d, points, nrm, dist_thresh, normal_cos, lo_iters=5):
    """Local optimization of RANSAC plane fit."""
    for _ in range(lo_iters):
        dists = np.abs(points @ n - d)
        dots = np.abs(nrm @ n)
        inlier = (dists < dist_thresh) & (dots > normal_cos)
        if inlier.sum() < 3:
            return n, d, inlier
...
def ransac_with_removal(points, nrm, indices,
                        ransac_iters=200, dist_thresh=0.05,
                        normal_cos=0.94, min_inliers=500,
                        max_planes=50, lo_iters=5):
    """
...
def merge_planes(planes, xyz, merge_compatible=None,
                 normal_thresh=0.95, rel_dist_thresh=0.02):
    """
...
def extract_planes_from_frame(depth, class_ids, valid, xyz, normals,
                              ...
                              ransac_iters=200, dist_thresh=0.05,
                              normal_cos=0.94, min_inliers=500,
                              max_planes=50, merge_normal=0.95,
                              merge_rel_dist=0.02):
    """
    ...
    for pid, p in enumerate(all_planes):
        for y, x in p['pixel_indices']:
            plane_map[y, x] = pid + 1  # 1-indexed to match existing convention
