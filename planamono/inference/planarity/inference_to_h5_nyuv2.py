#!/usr/bin/env python3
"""
MoGe Planarity Inference + Segmentation -> H5 (NYU-v2 Dataset)

Runs full pipeline for NYU-v2 dataset:
1. MoGe inference (planarity, depth, normals)
2. Vectorized segmentation
3. Saves planes.h5 to output directory (single virtual scene "nyuv2")

NYU-v2 data is stored as flat NPZ files (one file per sample, no scene
hierarchy). RGB is loaded from NPZ at 480x640 (high-res) resolution.

Usage:
    python inference_to_h5_nyuv2.py --model_path /path/to/model.pt --output_root /path/to/output
...
DEFAULT_CONFIG = {
    ...
    "output_root": "/cluster/scratch/aoezkan/planeseg/nyuv2/inference/moge_ours_h5",
    "data_root": "/cluster/scratch/aoezkan/planeseg/dataset/nyuv2_plane",

    # Inference parameters
    "num_tokens": 1024,
    "batch_size": 16,  # NYU-v2 images are 480x640
...
# ============================================================
# H5 SAVING (one planes.h5 per scene_id; NYU-v2 has a single scene "nyuv2")
# ============================================================
...
def save_predictions_h5(
    ...
    for scene_id, frame_data in tqdm(scene_predictions.items(), desc="Writing H5"):
        # Sort by integer frame index when possible (NYU-v2 frame_ids are numeric)
        try:
...
def process_batch(
    ...
    """
    Run batch inference and segmentation for NYU-v2.
    ...
    with timer("load_npz"):
        ...
        for formatted_path, scene_id, frame_id in zip(formatted_paths, scene_ids, frame_ids):
            # NYU-v2 npz layout is flat: "<idx>_d2.npz" (no split prefix)
            npz_file = f"{frame_id}_d2.npz"
    ...
    for res, (formatted_path, scene_id, frame_id) in zip(results, valid_meta):
        # NYU-v2 high-res is always 480x640
        H_rgb, W_rgb = 480, 640
...
def main():
    parser = argparse.ArgumentParser(
        description="MoGe Planarity Inference + Segmentation -> H5 (NYU-v2)",
    ...
    parser.add_argument("--data_root", type=str, default=DEFAULT_CONFIG["data_root"],
                        help="NYU-v2 dataset root (directory with <idx>_d2.npz files)")
    ...
    print("=" * 60)
    print("MoGe Planarity Inference + Segmentation -> H5 (NYU-v2)")
    print("=" * 60)
    ...
    def collate_fn(batch):
        ...
        if not batch:
            return None
        return {
            "rgb_path": [b["rgb_path"] for b in batch],     # "nyuv2/<split>_<idx>"
            "scene_id": [b["scene_id"] for b in batch],     # always "nyuv2"
