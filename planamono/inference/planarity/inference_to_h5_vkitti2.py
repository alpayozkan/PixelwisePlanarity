#!/usr/bin/env python3
"""
MoGe Planarity Inference + Segmentation -> H5 (VKITTI2 Dataset)

Runs full pipeline for VKITTI2 dataset:
1. MoGe inference (planarity, depth, normals)
2. Vectorized segmentation
3. Saves planes.h5 to output directory (one per scene_id = scene/variant)

Usage:
    python inference_to_h5_vkitti2.py --model_path /path/to/model.pt --output_root /path/to/output
...
DEFAULT_CONFIG = {
    ...
    "output_root": "/cluster/scratch/aoezkan/planeseg/vkitti2/inference/moge_ours_h5",
    "data_root": "/cluster/scratch/ayavuz/dataset/vkitti2_planes",

    # Inference parameters
    "num_tokens": 1024,
    "batch_size": 16,  # VKITTI2 images are 375x1242
...
# ============================================================
# H5 SAVING (ScanNet++-style: one planes.h5 per scene_id)
# ============================================================
...
def save_predictions_h5(
    ...
    Args:
        scene_predictions: {scene_id: [(frame_id, labels), ...]}
            scene_id is "scene/variant" (e.g., "Scene06/clone")
...
def process_batch(
    ...
    """
    Run batch inference and segmentation for VKITTI2.

    Returns list of dicts with scene_id, frame_id, labels.
    """
    # Parse formatted rgb_path strings (format: "scene/variant/frame_id")
    formatted_paths = batch["rgb_path"]
    scene_ids = batch["scene_id"]
    frame_ids = batch["frame_id"]

    # Load RGB images from H5 files
    with timer("load_h5"):
        rgb_images = []
        for formatted_path in formatted_paths:
            parts = formatted_path.split("/")
            scene, variant, fid = parts[0], parts[1], parts[2]

            h5_path = os.path.join(data_root, scene, variant, "scene_data.h5")
            try:
                with h5py.File(h5_path, "r") as f:
                    # Find frame index from frame_ids dataset
                    frame_ids_h5 = [
                        fi.decode("utf-8") if isinstance(fi, bytes) else str(fi)
                        for fi in f["frame_ids"][:]
                    ]
                    try:
                        frame_idx_int = frame_ids_h5.index(fid)
                    except ValueError:
                        frame_idx_int = int(fid) if fid.isdigit() else 0
                        print(f"[WARN] Frame {fid} not in frame_ids, using index {frame_idx_int}")

                    rgb = f["rgb"][frame_idx_int]  # (H, W, 3) uint8
                rgb_images.append(rgb)
            except Exception as e:
                print(f"[ERROR] Failed to load {h5_path} frame {fid}: {e}")
                rgb_images.append(np.zeros((375, 1242, 3), dtype=np.uint8))
    ...
    for res, formatted_path, scene_id, frame_id in zip(results, formatted_paths, scene_ids, frame_ids):
        ...
        with timer("postprocess_io"):
            with h5py.File(h5_path, "r") as f:
                H_rgb = int(f.attrs.get("image_height", f["rgb"].shape[1]))
                W_rgb = int(f.attrs.get("image_width", f["rgb"].shape[2]))
...
def main():
    parser = argparse.ArgumentParser(
        description="MoGe Planarity Inference + Segmentation -> H5 (VKITTI2)",
    ...
    parser.add_argument("--variants", nargs="+", default=None,
                        help="Variants to include (default: all). E.g., --variants clone fog morning")
    ...
    print("=" * 60)
    print("MoGe Planarity Inference + Segmentation -> H5 (VKITTI2)")
    print("=" * 60)
    print(f"Model:        {args.model_path}")
    print(f"Output:       {args.output_root}")
    print(f"Data root:    {args.data_root}")
    print(f"Split:        {args.split}")
    print(f"Variants:     {args.variants or 'all'}")
    ...
    def collate_fn(batch):
        return {
            "rgb_path": [b["rgb_path"] for b in batch],     # "scene/variant/frame_id"
            "scene_id": [b["scene_id"] for b in batch],     # "scene/variant"
            "frame_id": [b["frame_idx"] for b in batch],    # frame_id string
        }
    ...
    with timer("total_pipeline"):
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Processing")):
            batch_outputs = process_batch(batch, inference_model, args, timer, args.data_root, seg_fn=seg_fn)
