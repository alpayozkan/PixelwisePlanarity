"""
Run **default** MoGe-2 inference (no planarity head) and dump the signals
...
Output layout per scene:
    ...
    attrs:
        resolution           "HxW"
        num_tokens           int
        source_model         "Ruicheng/moge-2-vitl-normal"
...
Example
-------
python save_moge_signals.py \\
    --scenes planamono/splits/scannetpp/test.txt \\
    --output_root /cluster/scratch/aoezkan/planeseg/inference/moge_signals_v2vitl \\
...
def moge_forward_signals(
    model: MoGeModel,
    images: torch.Tensor,
    num_tokens: int,
) -> dict:
    """
    Run a MoGe-2 forward pass and produce the per-pixel + per-image signals
    we want to save for plane segmentation.
...
def process_scene(
    ...
    with h5py.File(out_h5, "w") as f:
        ...
        for i in tqdm(range(0, N, batch_size),
            ...
            imgs = _load_rgb_batch(paths, target_hw=target_hw).to(model.device)

            sig = moge_forward_signals(model, imgs, num_tokens=num_tokens)
