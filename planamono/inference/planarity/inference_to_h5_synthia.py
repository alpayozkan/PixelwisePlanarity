#!/usr/bin/env python3
"""
MoGe Planarity Inference + Segmentation -> H5 (Synthia Dataset)

Runs full pipeline for Synthia dataset:
1. MoGe inference (planarity, depth, normals)
2. Vectorized segmentation
3. Saves planes.h5 to output directory (one per scene)

Usage:
    python inference_to_h5_synthia.py --model_path /path/to/model.pt --output_root /path/to/output
...
DEFAULT_CONFIG = {
    ...
    "num_tokens": 1024,
    "batch_size": 16,  # Synthia images are 480x640
...
# ============================================================
# H5 SAVING (ScanNet++-style: one planes.h5 per scene)
# ============================================================
...
def save_predictions_h5(
    ...
    Args:
        scene_predictions: {scene_name: [(frame_id, labels), ...]}
...
def process_batch(
    ...
    """
    Run batch inference and segmentation for Synthia.

    Returns list of dicts with scene_id, frame_id, labels.
    """
    # Parse formatted rgb_path strings (format: "scene_name/frame_id")
    ...
    with timer("load_h5"):
        rgb_images = []
        for formatted_path, scene_id in zip(formatted_paths, scene_ids):
            parts = formatted_path.split("/")
            scene_name, fid = parts[0], parts[1]

            h5_path = os.path.join(data_root, split, scene_name, "scene_data.h5")
            try:
    ...
    for res, formatted_path, scene_id, frame_id in zip(results, formatted_paths, scene_ids, frame_ids):
        scene_name = formatted_path.split("/")[0]
        h5_path = os.path.join(data_root, split, scene_name, "scene_data.h5")
...
def main():
    parser = argparse.ArgumentParser(
        description="MoGe Planarity Inference + Segmentation -> H5 (Synthia)",
    ...
    print("=" * 60)
    print("MoGe Planarity Inference + Segmentation -> H5 (Synthia)")
    print("=" * 60)
    ...
    def collate_fn(batch):
        return {
            "rgb_path": [b["rgb_path"] for b in batch],     # "scene_name/frame_id"
            "scene_id": [b["scene_id"] for b in batch],     # scene_name
