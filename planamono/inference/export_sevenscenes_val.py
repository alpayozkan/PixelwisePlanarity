#!/usr/bin/env python3
"""
Export MoGe inference on the SevenScenes plane val split to a single H5 file.

Reads the JSON manifest (`sevenscenes_plane_len758_val.json`) and the per-frame
...
Usage:
    python planamono/inference/export_sevenscenes_val.py \
        --checkpoint /path/to/moge_planarity.pt \
        --output_dir /cluster/scratch/ayavuz/inference/<run>/sevenscenes
"""
...
DEFAULT_NPZ_ROOT = "/cluster/scratch/ayavuz/dataset/sevenscenes_plane"
DEFAULT_JSON = os.path.join(DEFAULT_NPZ_ROOT, "sevenscenes_plane_len758_val.json")

# npz GT layouts
GT_LOW_W,  GT_LOW_H  = 256, 192
GT_HIGH_W, GT_HIGH_H = 640, 480

# Max plane count to allocate for gt_planeparams (sevenscenes appears to cap ~20)
P_MAX = 21
...
def main():
    ...
    pbar = tqdm(total=N, desc="SevenScenes")
