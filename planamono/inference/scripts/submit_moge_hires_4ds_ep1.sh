#!/bin/bash
# Submit MoGe HiRes 4DS ep2 inference + evaluation pipeline on all 4 datasets:
...
METHOD="moge_hires_4ds_ep1"

echo "============================================================"
echo "Submitting $METHOD inference + evaluation on ALL 4 datasets"
echo "============================================================"
echo ""

# ── 1. Submit ScanNet++ inference (GPU) ──
JOB_SCANNETPP=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_4ds_ep1.sh")
echo "Submitted ScanNet++ inference: $JOB_SCANNETPP"

# ── 2. Submit Hypersim inference (GPU) ──
JOB_HYPERSIM=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_4ds_ep1_hypersim.sh")
echo "Submitted Hypersim  inference: $JOB_HYPERSIM"

# ── 3. Submit VKITTI2 inference (GPU) ──
JOB_VKITTI2=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_4ds_ep1_vkitti2.sh")
echo "Submitted VKITTI2   inference: $JOB_VKITTI2"

# ── 4. Submit Synthia inference (GPU) ──
JOB_SYNTHIA=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_4ds_ep1_synthia.sh")
...
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_scannetpp_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_scannetpp_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "ScanNet++ evaluation (moge_hires_4ds_ep1)"
python /cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono/evaluation/quantitative/evaluate_all_baselines.py \
    --methods moge_hires_4ds_ep1
...
#SBATCH --cpus-per-task=16
#SBATCH --mem-per-cpu=8G
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_hypersim_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_hypersim_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "Hypersim evaluation (moge_hires_4ds_ep1)"
...
#SBATCH --cpus-per-task=16
#SBATCH --mem-per-cpu=8G
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_vkitti2_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_vkitti2_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "VKITTI2 evaluation (moge_hires_4ds_ep1)"
python /cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono/evaluation/quantitative/evaluate_vkitti2_all_baselines.py \
    --methods moge_hires_4ds_ep1 --split test
...
#SBATCH --cpus-per-task=16
#SBATCH --mem-per-cpu=8G
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_synthia_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_4ds_ep1_synthia_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "Synthia evaluation (moge_hires_4ds_ep1)"
