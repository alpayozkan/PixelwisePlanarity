#!/bin/bash
# Submit MoGe HiRes ep1 inference + evaluation pipeline:
...
JOB_SCANNETPP=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_ep1.sh")
echo "Submitted ScanNet++ inference: $JOB_SCANNETPP"

# ── 2. Submit Hypersim inference (GPU) ──
JOB_HYPERSIM=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_ep1_hypersim.sh")
echo "Submitted Hypersim  inference: $JOB_HYPERSIM"

# ── 3. Submit ScanNet++ evaluation (CPU, depends on inference) ──
JOB_SCANNETPP_EVAL=$(sbatch --parsable --dependency=afterok:$JOB_SCANNETPP <<'EVAL_SCANNETPP'
#!/bin/bash
#SBATCH --time=12:00:00
#SBATCH --cpus-per-task=16
#SBATCH --mem-per-cpu=8G
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep1_scannetpp_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep1_scannetpp_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "ScanNet++ evaluation (moge_hires_ep1)"
python /cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono/evaluation/quantitative/evaluate_all_baselines.py \
    --methods moge_hires_ep1
...
# ── 4. Submit Hypersim evaluation (CPU, depends on inference) ──
JOB_HYPERSIM_EVAL=$(sbatch --parsable --dependency=afterok:$JOB_HYPERSIM <<'EVAL_HYPERSIM'
#!/bin/bash
#SBATCH --time=12:00:00
#SBATCH --cpus-per-task=16
#SBATCH --mem-per-cpu=8G
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep1_hypersim_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep1_hypersim_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "Hypersim evaluation (moge_hires_ep1)"
