#!/bin/bash
# Submit v5 segmentation ablation jobs (seg → eval → agg) for ScanNet++.
#
# Each variant runs as an independent job chain: seg → eval (array) → agg.
# Raw MoGe predictions are shared across all variants (Stage 1 already done).
#
# Usage:
#   bash submit_v5_ablation.sh                                          # all 4 variants
#   bash submit_v5_ablation.sh v5 v5_relative                           # specific variants
#   bash submit_v5_ablation.sh v5_no_sobel v5_dotprod_relative          # specific variants
#
# Variants (2x2 ablation: normal check x depth check):
#   v5                 — Sobel + absolute depth (original)
#   v5_relative        — Sobel + relative depth
#   v5_no_sobel        — dot-product + absolute depth
#   v5_dotprod_relative — dot-product + relative depth

set -euo pipefail

PROJECT_ROOT="/cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono"
LOG_DIR="/cluster/scratch/aoezkan/planeseg/logs/v5_ablation"
mkdir -p "$LOG_DIR"
...
# Shared segmentation parameters
THRESHOLD_PLANARITY=0.3
NORMAL_THRESHOLD_DEG=5.0
NEIGHBOR_MATCH_COUNT_THRESH=8   # v5 default
...
# ---- Variant definitions ----
# Maps variant name → (seg_version, depth_threshold, eval_method_key)
...
VARIANT_EVAL_METHOD[v5_dotprod_relative]="moge_hires_ep3_v5_dotprod_relative_seg"
...
echo "============================================================"
echo "v5 Segmentation Ablation (ScanNet++)"
echo "============================================================"
...
echo \"Eval scenes [\$SCENE_START:\$SCENE_END] for $VARIANT\"
