#!/bin/bash
# MoGe HiRes ep3 + v10 segmentation (adaptive threshold + small segment filter) — ScanNet++ Inference
#SBATCH --time=24:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem-per-cpu=16G
#SBATCH --gpus=rtx_3090:1
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep3_v10seg_scannetpp_infer_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep3_v10seg_scannetpp_infer_%j.err

set -euo pipefail

MODEL_PATH="/cluster/scratch/ayavuz/moge_mixed_output_476644_fixed_cosLR_singlePhase_mixed_HiRes/model_epoch3.pt"
SPLIT="test"
BATCH_SIZE=8
NUM_TOKENS=1600
THRESHOLD_PLANARITY=0.3
NORMAL_THRESHOLD_DEG=5.0
DEPTH_THRESHOLD=0.025
NEIGHBOR_MATCH_COUNT_THRESH=18

# v10-specific parameters
SEG_VERSION="v10"
ADAPTIVE_FRAC=0.75
MIN_VALID_NEIGHBORS=3
MIN_SEGMENT_PIXELS=50

PROJECT_ROOT="/cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono"
# OUTPUT_ROOT="/cluster/scratch/aoezkan/planeseg/scannetpp/inference/moge_hires_ep3_v10seg_h5"
OUTPUT_ROOT="/cluster/scratch/aoezkan/planeseg/scannetpp/inference/moge_hires_ep3_v10seg_metric_h5"
DATASET_DIR="/cluster/scratch/aoezkan/planeseg/dataset/scannetpp"
RGB_ROOT="/cluster/project/cvg/Shared_datasets/scannet++/data"

source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "ScanNet++ inference (v10 seg) → $OUTPUT_ROOT"
mkdir -p "$OUTPUT_ROOT"

python "$PROJECT_ROOT/inference/planarity/inference_to_h5.py" \
    --model_path "$MODEL_PATH" \
    --output_root "$OUTPUT_ROOT" \
    --dataset_dir "$DATASET_DIR" \
    --rgb_root "$RGB_ROOT" \
    --split "$SPLIT" \
    --batch_size "$BATCH_SIZE" \
    --num_tokens "$NUM_TOKENS" \
    --threshold_planarity "$THRESHOLD_PLANARITY" \
    --normal_threshold_deg "$NORMAL_THRESHOLD_DEG" \
    --depth_threshold "$DEPTH_THRESHOLD" \
    --neighbor_match_count_thresh "$NEIGHBOR_MATCH_COUNT_THRESH" \
    --seg_version "$SEG_VERSION" \
    --adaptive_frac "$ADAPTIVE_FRAC" \
    --min_valid_neighbors "$MIN_VALID_NEIGHBORS" \
    --min_segment_pixels "$MIN_SEGMENT_PIXELS" \
    --metric_depth

echo "[DONE] ScanNet++ inference (v10 seg)"

echo "Starting evaluation..."
python "$PROJECT_ROOT/evaluation/quantitative/evaluate_all_baselines.py" \
    --methods moge_hires_ep3_v10seg_metric

echo "[DONE] Evaluation complete"
