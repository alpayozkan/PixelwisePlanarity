#!/bin/bash
# Submit MoGe Focal HiRes ep1 inference + evaluation pipeline:
...
JOB_SCANNETPP=$(sbatch --parsable "$SCRIPT_DIR/run_moge_focal_hires_ep1.sh")
echo "Submitted ScanNet++ inference: $JOB_SCANNETPP"

# ── 2. Submit Hypersim inference (GPU) ──
JOB_HYPERSIM=$(sbatch --parsable "$SCRIPT_DIR/run_moge_focal_hires_ep1_hypersim.sh")
...
#SBATCH --gpus=1
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_focal_hires_ep1_scannetpp_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_focal_hires_ep1_scannetpp_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "ScanNet++ evaluation (moge_focal_hires_ep1)"
python /cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono/evaluation/quantitative/evaluate_all_baselines.py \
    --methods moge_focal_hires_ep1
...
#SBATCH --mem-per-cpu=8G
#SBATCH --gpus=1
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_focal_hires_ep1_hypersim_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_focal_hires_ep1_hypersim_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "Hypersim evaluation (moge_focal_hires_ep1)"
