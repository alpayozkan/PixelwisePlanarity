#!/bin/bash
# Submit MoGe HiRes ep2 inference + evaluation pipeline:
...
echo "============================================================"
echo "Submitting MoGe HiRes ep2: VKITTI2 + Synthia"
echo "============================================================"

# ── 1. Submit VKITTI2 inference (GPU) ──
JOB_VK_INFER=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_ep2_vkitti2.sh")
echo "Submitted VKITTI2  inference: $JOB_VK_INFER"

# ── 2. Submit Synthia inference (GPU) ──
JOB_SYN_INFER=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_ep2_synthia.sh")
...
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/eval_vkitti2/eval_moge_hires_ep2_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/eval_vkitti2/eval_moge_hires_ep2_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "VKITTI2 evaluation (moge_hires_ep2)"
python $EVAL_DIR/evaluate_vkitti2_all_baselines.py --methods moge_hires_ep2 --split test
...
#SBATCH --cpus-per-task=16
#SBATCH --mem-per-cpu=8G
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/eval_synthia/eval_moge_hires_ep2_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/eval_synthia/eval_moge_hires_ep2_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "Synthia evaluation (moge_hires_ep2)"
python $EVAL_DIR/evaluate_synthia_all_baselines.py --methods moge_hires_ep2 --split test
