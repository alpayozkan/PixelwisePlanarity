#!/bin/bash
# Submit 4ds ep2 + v5_relative inference + eval across all 4 datasets.
# Same segmentation params as submit_v5_relative_all_datasets.sh but with
# the 4-dataset trained checkpoint (moge_HIRES_4datasets/model_epoch2.pt).
#
# Methods:
#   moge_hires_4ds_ep2_v5_relative_seg       (plan=0.3, norm=5.0°, match=8,  depth_rel=0.025)
#   moge_hires_4ds_ep2_v5origparams_relative_seg (plan=0.6, norm=10.0°, match=24, depth_rel=0.025)
#
# Usage:
#   bash submit_v5_relative_4ds_all_datasets.sh

set -euo pipefail

PROJECT_ROOT="/cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono"
LOG_DIR="/cluster/scratch/aoezkan/planeseg/logs/v5_relative_4ds_all_datasets"
mkdir -p "$LOG_DIR"
...
declare -a METHODS=(
    "moge_hires_4ds_ep2_v5_relative_seg:0.3:5.0:8"
    "moge_hires_4ds_ep2_v5origparams_relative_seg:0.6:10.0:24"
)

echo "============================================================"
echo "v5_relative Ablation — 4ds ep2 — All Datasets"
echo "============================================================"
