#!/bin/bash
# Two-stage pipeline: GPU segmentation → distributed CPU evaluation (Hypersim)
# Usage: bash run_segment_eval_hypersim.sh [NUM_EVAL_JOBS]
...
echo "=== Submitting Hypersim pipeline ==="
...
SEG_JOB=$(sbatch --parsable \
    ...
    --mem-per-cpu=8G \
    --gpus=rtx_3090:1 \
    --output="$LOG_DIR/seg_hypersim_%j.out" \
    --error="$LOG_DIR/seg_hypersim_%j.err" \
    --wrap="
...
# Hypersim test split has 70 scenes. Divide among NUM_EVAL_JOBS.
TOTAL_SCENES=70
...
EVAL_JOB=$(sbatch --parsable \
    ...
    --cpus-per-task=16 \
    --mem-per-cpu=4G \
    --output="$LOG_DIR/eval_hypersim_%A_%a.out" \
    --error="$LOG_DIR/eval_hypersim_%A_%a.err" \
    --wrap="
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono
...
AGG_JOB=$(sbatch --parsable \
    ...
    --cpus-per-task=2 \
    --mem-per-cpu=4G \
    --output="$LOG_DIR/agg_hypersim_%j.out" \
    --error="$LOG_DIR/agg_hypersim_%j.err" \
    --wrap="
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono
python $PROJECT_ROOT/evaluation/quantitative/evaluate_hypersim_all_baselines.py \
