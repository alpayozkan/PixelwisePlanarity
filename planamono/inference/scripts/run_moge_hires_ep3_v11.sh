#!/bin/bash
# MoGe HiRes ep3 + v11 segmentation — ScanNet++ Inference → Distributed Eval
# Usage: bash run_moge_hires_ep3_v11.sh [NUM_EVAL_JOBS]
...
# v11 merge params (passed but merge_enabled=False by default)
MERGE_NORMAL_DEG=15.0
MERGE_OFFSET_M=0.05
MERGE_MIN_PIXELS=50
MERGE_GAP_PX=5
...
echo "=== Submitting ScanNet++ v11 pipeline ==="
...
INFER_JOB=$(sbatch --parsable \
    ...
    --mem-per-cpu=16G \
    --gpus=rtx_3090:1 \
    --output="$LOG_DIR/moge_hires_ep3_v11seg_scannetpp_infer_%j.out" \
    --error="$LOG_DIR/moge_hires_ep3_v11seg_scannetpp_infer_%j.err" \
    --wrap="
...
EVAL_JOB=$(sbatch --parsable \
    ...
    --cpus-per-task=16 \
    --mem-per-cpu=4G \
    --output="$LOG_DIR/eval_v11_scannetpp_%A_%a.out" \
    --error="$LOG_DIR/eval_v11_scannetpp_%A_%a.err" \
    --wrap="
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh || true
conda activate planamono
export HDF5_USE_FILE_LOCKING=FALSE
...
AGG_JOB=$(sbatch --parsable \
    ...
    --cpus-per-task=2 \
    --mem-per-cpu=4G \
    --output="$LOG_DIR/agg_v11_scannetpp_%j.out" \
    --error="$LOG_DIR/agg_v11_scannetpp_%j.err" \
    --wrap="
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh || true
conda activate planamono
export HDF5_USE_FILE_LOCKING=FALSE
