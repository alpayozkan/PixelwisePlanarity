#!/bin/bash
# Submit MoGe HiRes ep6 inference + evaluation pipeline:
...
JOB_SCANNETPP=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_ep6.sh")
echo "Submitted ScanNet++ inference: $JOB_SCANNETPP"

# ── 2. Submit Hypersim inference (GPU) ──
JOB_HYPERSIM=$(sbatch --parsable "$SCRIPT_DIR/run_moge_hires_ep6_hypersim.sh")
...
#SBATCH --gpus=1
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep6_scannetpp_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep6_scannetpp_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "ScanNet++ evaluation (moge_hires_ep6)"
python /cluster/home/aoezkan/planeseg/PixelwisePlanarity/planamono/evaluation/quantitative/evaluate_all_baselines.py \
    --methods moge_hires_ep6
...
#SBATCH --mem-per-cpu=8G
#SBATCH --gpus=1
#SBATCH --output=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep6_hypersim_eval_%j.out
#SBATCH --error=/cluster/scratch/aoezkan/planeseg/logs/inference/moge_hires_ep6_hypersim_eval_%j.err

set -euo pipefail
source /cluster/scratch/aoezkan/miniconda3/etc/profile.d/conda.sh
conda activate planamono

echo "Hypersim evaluation (moge_hires_ep6)"
