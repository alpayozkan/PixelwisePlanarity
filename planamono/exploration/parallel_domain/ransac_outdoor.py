...
Adapted from PixelwisePlanarityAmo/planamono/shared/outdoor/ransac_image.py.
...
Key features:
  ...
  - LO iterations (10) for refined plane fits
  - Union-find merging with absolute distance floor
"""
...
def merge_planes(planes, xyz, merge_compatible=None,
    ...
    for i in range(n_planes):
        for j in range(i + 1, n_planes):
            ...
            ci = pi.get('class_id', -1)
            cj = pj.get('class_id', -2)
            can_merge = (ci == cj)
            if not can_merge:
                for group in merge_compatible:
                    if ci in group and cj in group:
                        can_merge = True
                        break
            if not can_merge:
                continue
