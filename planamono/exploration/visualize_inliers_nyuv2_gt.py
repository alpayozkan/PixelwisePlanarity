"""
Visualize inliers/outliers of NYU-v2 GT planes and dump full per-frame metrics.

For N random frames from NYUv2PlaneDataset:
...
Usage:
    python planamono/exploration/visualize_inliers_nyuv2_gt.py
    python planamono/exploration/visualize_inliers_nyuv2_gt.py --n-samples 20 --seed 0
    python planamono/exploration/visualize_inliers_nyuv2_gt.py --output-root /tmp/foo
"""
...
from planamono.paths import nyuv2_path
...
DEFAULT_OUTPUT_ROOT = Path(
    "/cluster/scratch/aoezkan/planeseg/exploration/nyuv2_gt_inliers"
)
...
def render_frame_png(out_path, rgb_np, depth_np, gt_seg, inlier_masks_per_thr,
                     predicted_mask, frame_idx, stats_per_thr):
    ...
    axes[0].set_title(f"RGB (idx={frame_idx})")
...
def parse_args():
    p = argparse.ArgumentParser(description="NYU-v2 GT inlier visualization + metrics")
    p.add_argument("--data-root", default=nyuv2_path,
                   help=f"NYU-v2 dataset root (default: {nyuv2_path})")
    ...
    p.add_argument("--exp-name", default=None,
                   help="Subfolder name (overwritten on re-run). "
                        "Default: nyuv2_gt_all OR nyuv2_gt_n<N>_seed<S>")
...
def main():
    ...
    if args.exp_name:
        ...
        exp_name = "nyuv2_gt_all"
    else:
        exp_name = f"nyuv2_gt_n{args.n_samples}_seed{args.seed}"
    out_dir = args.output_root / exp_name
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"Output: {out_dir}", flush=True)

    ds = NYUv2PlaneDataset(
    ...
    for k, idx in enumerate(tqdm(sample_indices, desc="frames")):
        ...
        if verbose:
            tqdm.write(f"\n=== Frame {k+1}/{len(sample_indices)} (sample idx={idx}, frame_idx={frame_idx}) ===")
        ...
        if save_png:
            ...
            render_frame_png(
                ...
                frame_idx=frame_idx, stats_per_thr=stats_per_thr,
            )
            if verbose:
    ...
    txt_lines = []
    txt_lines.append(f"NYU-v2 GT inlier evaluation -- {timestamp}")
    ...
    print("\n" + "=" * 80, flush=True)
    print(f"Evaluated {len(sample_indices)} frames, saved {n_pngs} PNGs", flush=True)
